...
```json
{
  ...
  "metadata": {
    "cycleId": "curate-...",
    "shipment": {
      "id": "shipment-stable-source-key",
      "decision": "ship",
      "rank": 0.8,
      "reason": "agent-authored reason this belongs"
    },
    "thread": {
...
- Every selected root item should carry a hidden `metadata.shipment.id` that is
  stable for its canonical source identity. Shipment rank/reason are supporting
  agent evidence, never a deterministic ordering formula.
- A standalone root item has no visible `metadata.thread`. Arrangement gives it
  `shipment-singleton:<feed-item-id>` as an internal shipment boundary. Visible
  thread metadata and chrome require a truthful cluster of at least two unique
  root members, all using the same stable thread ID through submit and arrange.
...
- For any accepted, visually dominant lead-level thread from a configured source,
  use direct story wording in `metadata.thread.threadTitle`, make
  `metadata.thread.threadRationale` one plain sentence about what happened, and
  set `metadata.thread.prominence.level` to `"lead"`.
...
- `analysis`: write original markdown and end with a `## Sources` section. Each source MUST be a markdown hyperlink: `[Source Title](https://url)`. Example: `- [Example publication: Interoperability analysis](https://example.com/analysis)`. Plain text citations without URLs are not acceptable.
...
The proposed interoperability rule could reduce switching costs, but adoption
depends on how quickly major implementers update their products.

## Sources
- [Example publication: Proposed interoperability rule](https://example.com/proposal)
- [Example standards body: Implementation timeline](https://example.org/timeline)
```
...
```json
{
  ...
  "type": "analysis",
  "source": "claude",
  "sourceId": "analysis-interoperability-001",
  "parentId": "feed-db-id-of-primary-source",
  "relationship": "analysis",
  "title": "Why implementation timing matters",
  "text": "## Why This Matters\n...\n\n## Sources\n- [Primary source](https://example.com/source)",
  "reason": "Connects the proposed rule to adoption constraints.",
  "mediaUrls": [
...
```json
{
  ...
  "publishedAt": "2020-01-01T11:30:00Z",
  "metadata": {
    "publishEvidence": {
      "status": "verified",
...
```json
{
  ...
  "type": "analysis",
  "source": "claude",
  "sourceId": "analysis-web-roundup-001",
  "parentId": null,
  "relationship": null,
  "title": "What the latest reporting says",
  "text": "## Why This Matters\n...\n\n## Sources\n- [Example publication](https://example.com/report)\n- [Example wire service](https://example.org/update)",
  "reason": "Synthesizes the clearest reporting threads from current coverage.",
  "mediaUrls": [
    "https://example.com/images/report-og.jpg"
  ],
...
- `metadata.suggestionType`: `"code_fix"`, `"life_admin"`, `"source_setup"`, or omitted for generic suggestions

`source_setup` cards come from the phone-paradigm source scout (usage stats + fresh installs
intersected with a content-app catalog). Two flavors:
...
Either way the feed row's `sourceId` (`source-scout-<package>`) blocks re-announcing/re-asking
via submit dedup, and browsing is read-only, public-surfaces-only, never DMs.

Fields every suggestion type may carry:

- `metadata.importance`: `"high"` or `"normal"` (default). High means time-critical or money at
  stake; arrange places high-importance suggestions above ALL content at the top of the feed,
  normal ones just below the first content block, and pending `code_fix` items at the back
  (dev backlog). Suggestions render inline in the main feed — placement is the only lever.
- `metadata.actions`: optional array of 1-3 agent-authored approval buttons that replace the
  generic Approve. Describe the concrete actions the user most plausibly wants:

```json
"actions": [
  { "label": "Send this reply", "instruction": "Send the exact draft in the card text via ...", "kind": "execute" },
  { "label": "Remind me Aug 9", "instruction": "Create a reminder suggestion on Aug 9 restating this card.", "kind": "execute" },
  { "label": "Mark handled", "kind": "acknowledge" }
]
```

  - `label`: button text, an imperative verb phrase under 48 chars, specific to THIS card
    ("Send this reply", "Turn off auto-renew") — never a bare "Accept".
  - `instruction`: self-contained execution text for that action; required for `kind: "execute"`.
    Only the card's own stored instruction is ever executed — the tap sends just the label.
  - `kind`: `"execute"` (dispatches the guardrailed life-actions agent; life_admin only) or
    `"acknowledge"` (marks the card accepted, nothing runs).
  - Every action must respect the life-admin guardrails: prevent-charge only, never move money,
    drafts to be sent must appear VERBATIM in the card text before offering a send action.
  - Cards without `actions` keep the classic Approve button driven by `metadata.executionSpec`.
...
A one-off/manual curation pass should make its final selected submit and summary
one coherent receipt. Do not call it successful unless the response is 2xx,
contains no item/system errors or rejected/deferred completion, and reports
`accepted + duplicates == cycleSummary.selected`. Only `accepted` is newly
shipped. Automated scheduler cycles additionally use the exact pending cycle ID
so the durable curation log can reach the matching terminal state.

After submit, explicitly call `/api/internal/curate/arrange`, including for an
empty pass. Preserve the agent's explicit shipment order and truthful 2+ member
threads; use hidden singleton shipment IDs for standalone roots. A successful
arrange receipt must confirm `ok: true` and a complete eligible/unviewed review.
The endpoint's structural merge may append omitted eligible shipments without
making a freshness or editorial judgment.
