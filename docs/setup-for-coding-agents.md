...
This guide installs the local/legacy-VM profile. Android is the canonical
production environment; for a phone use
[`docs/phone-production.md`](phone-production.md) and
[`phone-paradigm/device/MIGRATE-TO-NEW-PHONE.md`](../phone-paradigm/device/MIGRATE-TO-NEW-PHONE.md).

If you are a coding agent installing the local/VM profile, follow this guide. It
covers everything from system setup to personalization.
...
- **Low**: Minimal usage with faster or lower-cost models. Good for casual use or a constrained provider allowance.
- **Medium (recommended)**: Balanced usage and model quality. Good for most users.
- **High**: Maximum quality and the highest allowed usage.

Subscription guidance:

- Describe the relative quality, usage, and latency tradeoff using the provider's
  current published plan limits; pricing and plan names change and are not product
  defaults.
...
Use this exact warning: "You must configure at least one content source or the feed will be empty."
