...
> **Historical VM-era hardening plan.** Its least-privilege principles still
> matter, but its service layout, Bird proxy, root paths, and Redis assumptions
> are not the phone production architecture. See
...
GET /api/internal/twitter/user-tweets?username=<username>&n=10
...
Keep threat-model research notes and deployment probes in private local evidence.
Public security behavior is established by the repository's security tests,
least-privilege defaults, and the maintained security documentation.
