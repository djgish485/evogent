...
> **Legacy/demo profile.** OpenClaw is not the canonical phone curation owner.
> Phone production uses the Termux scheduler, a hint-free runtime brain, and the
> phone-local submit API. See
> [`docs/phone-production.md`](phone-production.md).

The optional VM/demo profile integrates with OpenClaw in two ways:
...
See `docs/openclaw-curator-migration.md` for that legacy curator runtime model.
