...
"""Crash-safe global budget for automatic barren-source diagnosis.
...
one automatic diagnosis slot per local service date.
...
def _empty_state() -> dict[str, Any]:
    return {"schemaVersion": SCHEMA_VERSION, "claims": {}, "sources": {}}
...
def claim_automatic_diagnosis(
    ...
    *,
    ...
    """Request the current threshold and atomically consume today's slot.

    Calling on every barren count is intentional.  ``handledThrough`` preserves
...
def clear_source_state(state_path: Path | str, *, source: str) -> bool:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    claim.add_argument("--barren-count", required=True, type=int)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "claim":
            ...
            clear_source_state(args.state, source=args.source)
        else:
            reset_source_streak(
                args.state,
                source=args.source,
