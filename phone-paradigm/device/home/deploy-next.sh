...
# a BUILD_ID (a broken build that won't serve) — this happened twice on 2026-07-18 from
# overlapping manual + finalize calls. flock serializes deploys; a second one waits, then no-ops
# if the staged tar is already live.
exec >> ~/deploy.log 2>&1
exec 9> ~/.deploy-next.lock
if ! flock -n 9; then
  echo "=== deploy $(date): another deploy holds the lock — waiting ==="
  flock 9
fi
echo "=== deploy $(date) ==="
cd ~/evogent || exit 1
...
# Already serving this exact build? Skip the churn (a queued duplicate deploy).
...
# POST-SWAP VALIDATION: if the swap didn't leave a valid BUILD_ID, roll back immediately rather
# than restart the server onto a broken build (the failure this hardening exists to prevent).
...
sleep 2
tmux new-session -d -s evo "cd ~/evogent && NODE_ENV=production PORT=3001 node server.js >> ~/evo-boot.log 2>&1"
...
echo "=== deploy done ==="
/data/data/com.termux/files/home/restart-evo.sh
