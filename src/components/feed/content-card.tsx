...
import { getYouTubeCanonicalSourceFields, getYouTubeFeedData, isYouTubeSource, type YouTubeFeedData } from '@/lib/youtube-feed';
...
import { openSourceUrl } from '@/lib/open-source-url';
...
const sourceAvatarMap: Record<string, SourceAvatarProfile> = {
  ...
  gmail: { initials: 'GM', displayName: 'Gmail', avatarClassName: 'bg-red-500 text-white' },
...
const sourceAliasMap: Record<string, string> = {
  ...
  hn: 'hackernews',
  googlemail: 'gmail',
  // Phone-captured sources (background browsing) render with their base identity.
  youtubephone: 'youtube',
  gmailphone: 'gmail',
  twitterphone: 'twitter',
  substackphone: 'substack',
...
function formatFeedTimestamp(item: Pick<FeedItem, 'publishedAt' | 'createdAt' | 'source' | 'type'>): string {
  ...
  // Preserve source publication time; ingestion time is not a valid age substitute.
...
function resolveFeedCardContextReason(
  item: Pick<FeedItem, 'displaySubtitle' | 'reason' | 'title' | 'text' | 'excerpt' | 'metadata'>,
): string | null {
  const metadataBridge = typeof item.metadata?.bridge === 'string' ? item.metadata.bridge.trim() : '';
  // Prefer explicit bridge metadata, then fall back to the learned-interest explanation.
  const rawInterestReason = typeof item.metadata?.interest?.reason === 'string' ? item.metadata.interest.reason.trim() : '';
  // Legacy floor rows carried machine provenance, which is bookkeeping rather than a public
  // explanation. V1 shipment rows instead carry the runtime agent's validated public reason.
  const explicitShipmentReason = item.metadata?.shipment?.decision === 'ship'
    && item.metadata.shipment.reason === rawInterestReason;
  const interestReason = item.metadata?.freshnessFloor === true && !explicitShipmentReason
    ? ''
    : rawInterestReason;
  const currentInterestReason = typeof item.metadata?.currentInterestReason === 'string' ? item.metadata.currentInterestReason.trim() : '';
  const reason = item.displaySubtitle?.trim() || item.reason?.trim() || metadataBridge
    || interestReason || currentInterestReason || null;
  if (!reason) {
    return null;
  }

  const duplicateText = [item.title, item.text, item.excerpt]
    .map((value) => value?.trim())
    .filter((value): value is string => Boolean(value));
  if (duplicateText.includes(reason)) {
    return null;
  }
...
function readSkillSlug(item: Pick<FeedItem, 'source' | 'metadata'>): string | null {
...
function resolvePrimaryLinkUrl(
  ...
  if (!isExternalWebUrl(itemUrl)) {
    return null;
  }
  if (sourceKey === 'hackernews' && canonical && isExternalWebUrl(canonical) && canonical !== itemUrl) {
    return canonical;
  }
...
function isExternalWebUrl(value: string | null | undefined): value is string {
...
// Tapping a content card now opens the item in its SOURCE APP (X → the tweet, YouTube →
// the video, browser → the article) instead of the in-app detail screen, so the per-source
// "Open on X / Watch on YouTube / Read original" buttons are redundant. Flag kept for easy
// revert.
const SHOW_SOURCE_OPEN_BUTTONS: boolean = false;

// The status id of a tweet, from its sourceId / url / metadata. Null for phone-scraped
// tweets ("phone-twitter-<handle>-<slug>", url=profile) that never captured a status id.
function resolveTweetStatusId(
  item: Pick<FeedItem, 'sourceId' | 'url' | 'metadata'>,
): string | null {
  const fromField = (value: string | null | undefined): string | null => {
    const trimmed = String(value ?? '').trim();
    if (!trimmed) return null;
    const prefixed = trimmed.match(/^tweet-(\d+)$/i);
    if (prefixed?.[1]) return prefixed[1];
    const status = trimmed.match(/\/status\/(\d+)/);
    if (status?.[1]) return status[1];
    return /^\d{6,}$/.test(trimmed) ? trimmed : null;
  };
  return fromField(item.sourceId)
    ?? fromField(item.url)
    ?? fromField(readMetadataString(item.metadata, 'tweetId'))
    ?? fromField(readMetadataString(item.metadata, 'statusId'))
    ?? fromField(readMetadataString(item.metadata, 'tweet_id'))
    ?? fromField(readMetadataString(item.metadata, 'status_id'));
}

// Where tapping a tweet should land in X: the exact tweet when we have a status id,
// otherwise the item URL (the author profile) as the best-available X destination.
function resolveTweetOpenUrl(
  item: Pick<FeedItem, 'sourceId' | 'url' | 'authorUsername' | 'metadata' | 'text' | 'title'>,
): string | null {
  const statusId = resolveTweetStatusId(item);
  const handle = item.authorUsername?.trim();
  const handleUncertain = item.metadata?.handleUncertain === true;
  if (statusId) {
    return handle && !handleUncertain
      ? `https://x.com/${handle}/status/${statusId}`
      : `https://x.com/i/web/status/${statusId}`;
  }
  // A stored source URL is evidence; an author-looking string is not. In particular, the phone
  // scraper deliberately emits url=null when X exposes only a display name and the derived
  // authorUsername is a guess. Reconstructing x.com/<guess> here would undo that honesty and send
  // a card tap to an unrelated or nonexistent profile. With no verified URL, stay in-app.
  const itemUrl = item.url?.trim();
  if (handleUncertain && itemUrl && !/\/status\/\d+/.test(itemUrl)) return null;
  if (itemUrl && isExternalWebUrl(itemUrl)) return itemUrl;
  return null;
}

// The external destination for tapping a content card, or null to keep the in-app detail
// view. Uses the same structural signal that gates the source-open button today
// (resolvePrimaryLinkUrl truthy = external content), so agent-authored articles, analyses,
// suggestions, and notifications (no external web URL) stay in-app.
export function resolveExternalOpenUrl(
  item: Pick<FeedItem, 'type' | 'source' | 'sourceId' | 'url' | 'authorUsername' | 'metadata' | 'text' | 'title'>,
): string | null {
  // Only X content takes the tweet-resolution path (whose fallback is an x.com search).
  // Other social sources shipped as type 'tweet' (e.g. Instagram posts) must open their own
  // URL, never an X search for their caption.
  const sourceKey = normalizeSourceKey(item.source);
  if (sourceKey === 'twitter' || (item.type === 'tweet' && sourceKey !== 'instagram')) {
    return resolveTweetOpenUrl(item);
  }
  const canonical = readMetadataString(item.metadata, 'canonicalUrl')?.trim();
  if (canonical && isExternalWebUrl(canonical)) return canonical;
  // YouTube items can carry a videoId (in sourceId/metadata) but NO url — they render fine from
  // that id but body-tap resolved to null and fell to the in-app detail instead of opening the
  // video. Reconstruct the canonical watch URL from the id.
  if (normalizeSourceKey(item.source) === 'youtube') {
    const yt = getYouTubeCanonicalSourceFields(item);
    if (yt?.canonicalUrl && isExternalWebUrl(yt.canonicalUrl)) return yt.canonicalUrl;
  }
  return resolvePrimaryLinkUrl(item);
}
...
export function resolveArticleHeaderSubline(
  ...
  if (isAgentCreatedSource(item.source)) {
...
/** A card must never introduce itself as "unknown" (a real feed led with one). When author
 *  fields are empty, the content's own site/source is the honest byline. */
function authorFallbackLabel(item: { source?: string | null; url?: string | null }): string {
...
/** Author links go to the author's profile on the content's OWN platform — an Instagram author
 *  used to link to x.com/<handle>, someone else entirely. */
function authorProfileUrl(item: Pick<FeedItem, 'source' | 'authorUsername'>): string {
  if (item.source === 'instagram') return `https://www.instagram.com/${item.authorUsername}/`;
  return `https://x.com/${item.authorUsername}`;
}

/** The @handle line under the name is only informative when it differs from the display name —
 *  "exampleauthor" over "@exampleauthor" is pure noise. */
function handleDiffersFromName(displayName: string, username: string): boolean {
  const norm = (s: string) => s.replace(/[^a-z0-9]/gi, '').toLowerCase();
  return norm(displayName) !== norm(username);
}
...
function MediaDisplay({
  ...
  prominent?: boolean;
  /** Image tiles link straight to the post/story URL (opens the source app on device) instead
   *  of the zoom lightbox — the Instagram experience: tapping the photo takes you to it. */
            ...
            <div key={`${entry.url}-${index}`} className={`overflow-hidden bg-zinc-900 ${tileClass}`}>
              {isVideo ? (
                ...
                <a
                  ...
                  onClick={(event) => event.stopPropagation()}
                  aria-label="Open post in its app"
                >
                  <ResilientImage
                    src={mediaSrc}
                    alt={mediaAlt}
...
function ParentTweetPreview({
  ...
  const username = item.authorUsername || authorFallbackLabel(item);
...
export function TweetCard({
  ...
  const primaryLinkUrl = resolvePrimaryLinkUrl(item);
  ...
  const feedContextReason = hideFeedContextReason ? null : resolveFeedCardContextReason(item);
  ...
  const metricsRow = (
    ...
    >
      ...
      {typeof item.metrics.views === 'number' && item.metrics.views > 0 ? (
        <MetricButton
          icon={<ViewsIcon />}
          count={item.metrics.views}
          hoverColor="group-hover:text-zinc-300 group-hover:bg-zinc-500/10"
          disabled
          ariaLabel="Views"
          className="shrink-0"
        />
  ...
  const openLinkRow = SHOW_SOURCE_OPEN_BUTTONS && (primaryLinkUrl || secondaryLink) ? (
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              {handleDiffersFromName(displayName, username) && (
          ...
          {feedContextReason && (
            <p data-testid="feed-card-context-reason" className="text-[15px] leading-7 text-zinc-400 sm:text-base">
              <HighlightedSearchText text={feedContextReason} searchQuery={searchQuery} />
            </p>
          )}
          ...
          {media.length > 0 && <MediaDisplay media={media} tweetUrl={item.url || undefined} onImageClick={onImageClick} prominent openMediaExternally={item.source === 'instagram'} />}
  ...
  return (
    <>
      ...
      <div>
        ...
        {feedContextReason && (
          <p data-testid="feed-card-context-reason" className="mt-3 text-[14px] leading-5 text-zinc-400">
            <HighlightedSearchText text={feedContextReason} searchQuery={searchQuery} />
          </p>
        )}

        {displayText && (
          <div className={`relative select-text touch-auto ${feedContextReason ? 'mt-2' : isProminent ? 'mt-4' : 'mt-3'}`}>
        ...
        {item.metadata?.communityNote && <CommunityNoteCallout note={item.metadata.communityNote} />}
        {item.metadata?.poll && <TweetPoll poll={item.metadata.poll} />}
        {media.length > 0 && <MediaDisplay media={media} tweetUrl={item.url || undefined} onImageClick={onImageClick} openMediaExternally={item.source === 'instagram'} />}
        {linkPreviews.map((preview) => <LinkPreviewCard key={preview.url} card={preview} />)}
        {item.metadata?.quotedTweet && (
          <QuoteTweetCard
            quote={item.metadata.quotedTweet}
...
export function ArticleCard({
  ...
  const feedContextReason = hideFeedContextReason ? null : resolveFeedCardContextReason(item);
  const isProminent = isProminentFeedItem(item);
  const linkUrl = youtubeVideo?.canonicalUrl ?? resolvePrimaryLinkUrl(item);
  ...
  const openLinkRow = SHOW_SOURCE_OPEN_BUTTONS && (linkUrl || secondaryLink) ? (
  ...
  if (isAgentSessionArticleLayout(item)) {
    ...
    const footerParts = [
      ...
      resolveArticleHeaderSubline(item, agentName),
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {feedContextReason && (
              <p data-testid="feed-card-context-reason" className={`mt-2 text-zinc-400 ${isProminent ? 'text-[15px] leading-7' : 'text-[14px] leading-relaxed'}`}>
                <HighlightedSearchText text={feedContextReason} searchQuery={searchQuery} />
              </p>
            )}
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-sm text-zinc-500">{resolveArticleHeaderSubline(item, agentName)}</p>
        </div>
      </div>
      ...
      {feedContextReason ? (
        <p data-testid="feed-card-context-reason" className="text-sm leading-6 text-zinc-400">
          <HighlightedSearchText text={feedContextReason} searchQuery={searchQuery} />
        </p>
      ) : null}
...
export function ContentCard({
  ...
  const openClawSkillSlug = useMemo(() => readSkillSlug(item), [item]);
  ...
  const openFeedItemPrimaryAction = useCallback((feedItem: FeedItem) => {
    // External content (tweet/youtube/article/web) opens in its source app; internal
    // items (agent analyses, suggestions, notifications) keep the in-app detail view.
    const externalUrl = resolveExternalOpenUrl(feedItem);
    if (externalUrl) {
      recordExpandInteraction(feedItem.id);
      openSourceUrl(externalUrl);
      return;
    }
        ...
        <div className="relative">
          ...
          <div className="relative z-10">
            <TweetCard
              ...
              hideFeedContextReason={hideFeedContextReason}
              expanded={isExpanded}
