...
interface SuggestionCardProps {
  ...
  onAccept: (item: FeedItem, action?: SuggestionAction) => void;
...
export function SuggestionCard({
  ...
  // Tapping the card title opens the source Evogent read. Only http(s) links are accepted;
  // the WebView shell routes them out to the source app.
  const sourceUrlRaw = typeof item.metadata?.sourceUrl === 'string' ? item.metadata.sourceUrl.trim() : '';
  const sourceUrl = /^https?:\/\//i.test(sourceUrlRaw) ? sourceUrlRaw : null;
  const executionResult = typeof item.metadata?.executionResult === 'string' && item.metadata.executionResult.trim()
    ? item.metadata.executionResult.trim()
    : null;
  const executionResultStatus = typeof item.metadata?.executionResultStatus === 'string'
    ? item.metadata.executionResultStatus
    : null;
  const executionResultUrlRaw = typeof item.metadata?.executionResultUrl === 'string' ? item.metadata.executionResultUrl.trim() : '';
  const executionResultUrl = /^(https?:|mailto:)/i.test(executionResultUrlRaw) ? executionResultUrlRaw : null;
  ...
  // Agent-authored approval buttons ("Send this reply") replace the generic Approve when present.
  const suggestionActions = isCodeFix ? [] : getSuggestionActions(item);
  ...
  return (
    <>
      <article
        ...
        // The whole card surface taps through to the source (the email/page this suggestion is
        // about) — matching every other card's behavior. Only the linked title was tappable
        // before, so tapping the card body did not open the source. Interactive children
        // (buttons, links, inputs) keep their own behavior via the closest() guard.
        onClick={sourceUrl ? (event) => {
      ...
      >
        ...
        <div className="relative pl-2 sm:pl-3">
            ...
            <h3 className="text-[15px] font-semibold leading-5 text-zinc-100 sm:text-base sm:leading-6">
              {sourceUrl ? (
                <a
                  ...
                  data-testid="suggestion-source-link"
                >
          ...
          {executionResult ? (
            // The outcome of an approved action, reported back by the execution agent — the
            // card is the only surface the user watches, so the full result (e.g. the prepared
            // draft) renders here, with a tap-through when the agent supplied a next step
            // (a mailto: compose link opens the mail app with the draft filled in).
            <div
              className={`mt-3 rounded-xl border px-3 py-2.5 ${executionResultStatus === 'failed'
                ? 'border-red-800/60 bg-red-950/20'
                : 'border-emerald-800/60 bg-emerald-950/15'}`}
              data-testid="suggestion-execution-result"
            >
              <p className="whitespace-pre-wrap text-sm leading-5 text-zinc-200">{executionResult}</p>
              {executionResultUrl ? (
                <a
                  href={executionResultUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-2 inline-flex min-h-9 items-center rounded-full border border-emerald-700/70 bg-emerald-950/32 px-4 py-1.5 text-sm font-medium text-emerald-100 transition-colors hover:border-emerald-500/80 hover:bg-emerald-900/45"
                  data-testid="suggestion-execution-result-link"
                >
                  {executionResultUrl.startsWith('mailto:') ? 'Open draft in email' : 'Open'}
                </a>
              ) : null}
            </div>
          ) : status === 'dispatched' ? (
            <p className="mt-3 text-xs text-amber-200/90" data-testid="suggestion-execution-pending">
              Working on it — the result will appear here.
            </p>
          ) : null}
          ...
          {!hideable && status !== 'dismissed' ? (
            <div className="mt-4 flex flex-wrap items-center gap-2">
              {suggestionActions.length > 0 ? (
                suggestionActions.map((action, actionIndex) => {
                  const buttonClass = actionIndex === 0
                    ? 'min-h-10 rounded-full border border-emerald-700/70 bg-emerald-950/32 px-4 py-2 text-sm font-medium text-emerald-100 transition-colors hover:border-emerald-500/80 hover:bg-emerald-900/45 disabled:cursor-not-allowed disabled:opacity-65'
                    : 'min-h-10 rounded-full border border-emerald-800/50 bg-transparent px-4 py-2 text-sm font-medium text-emerald-200/90 transition-colors hover:border-emerald-500/70 hover:bg-emerald-950/40 disabled:cursor-not-allowed disabled:opacity-65';
                  // A LINK action just opens its page (routed out to the source app by the shell)
                  // and leaves the card exactly where it is — the user is going to LOOK, not
                  // resolve. Only execute/acknowledge run through onAccept.
                  if (action.kind === 'link' && action.url) {
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        data-testid={`suggestion-link-button-${actionIndex}`}
                        onClick={(event) => {
                          event.stopPropagation();
                          event.preventDefault();
                          openSourceUrl(action.url);
                        }}
                  ...
                  return (
                    <button
                      key={`${action.label}:${actionIndex}`}
                      type="button"
                      data-testid={actionIndex === 0 ? 'suggestion-accept-button' : `suggestion-action-button-${actionIndex}`}
                      disabled={disabled || !canAccept}
                      onClick={() => onAccept(item, action)}
                      className={buttonClass}
                    >
                      {pendingAction === 'accept' ? 'Working...' : action.label}
                    </button>
                  );
                })
              ) : (
              ...
              {onChatAboutSuggestion ? (
                <button
                  type="button"
                  data-testid="suggestion-chat-button"
                  aria-label="Chat about this suggestion"
                  disabled={disabled}
                  onClick={() => onChatAboutSuggestion(item)}
                  className="inline-flex min-h-10 items-center gap-2 rounded-full border border-sky-800/60 bg-sky-950/28 px-4 py-2 text-sm font-medium text-sky-100 transition-colors hover:border-sky-500/80 hover:bg-sky-900/40 disabled:cursor-not-allowed disabled:opacity-65"
                >
                  {renderActionIcon('chat')}
                  <span>Chat</span>
                </button>
              ) : null}
            </div>
