...
import { hasTruthfulThreadMembers } from '@/lib/feed-render-entries';
...
function renderContentCard(
  ...
  // Suppress the card's own context-reason line only when the thread actually rendered a
  // bridge line above it (real duplication) — never as a blanket flag. With no bridge, the
  // card's own reason chain is the only place "why this is here" can show.
  const card = (
    <ContentCard
      key={item.id}
      item={item}
      suppressedChildPreviewIds={options?.suppressedChildPreviewIds}
      hideFeedContextReason={Boolean(options?.threadTint && getItemBridge(item))}
      agentName={agentName}
      onChat={onChat}
...
function getItemBridge(item: FeedItem): string {
  if (typeof item.displaySubtitle === 'string' && item.displaySubtitle.trim()) {
    return item.displaySubtitle.trim();
  }
  const bridge = typeof item.metadata?.bridge === 'string' ? item.metadata.bridge.trim() : '';
  if (bridge) return bridge;
  // Keep thread context concise: explicit bridge first, then the item reason.
  if (typeof item.reason === 'string' && item.reason.trim()) return item.reason.trim();
  const interestReason = item.metadata?.interest?.reason;
  // Suppress machine provenance on legacy floor rows, but show the validated runtime agent's
  // public explanation on explicit v1 shipments.
  const explicitShipmentReason = item.metadata?.shipment?.decision === 'ship'
    && item.metadata.shipment.reason === interestReason;
  if (typeof interestReason === 'string' && interestReason.trim()
    && (item.metadata?.freshnessFloor !== true || explicitShipmentReason)) return interestReason.trim();
  const currentInterestReason = item.metadata?.currentInterestReason;
  if (typeof currentInterestReason === 'string' && currentInterestReason.trim()) return currentInterestReason.trim();
  return '';
}
...
export function ThreadGroup({
  ...
  // Defense in depth: callers should already leave a singleton as a normal feed entry, but stale
  // or migrated metadata must never manufacture a Thread header or collapse affordance.
  if (!hasTruthfulThreadMembers(items)) {
    return (
      <section data-testid="truthful-singleton-feed-entry" data-feed-stability-anchor className="space-y-3 pt-4">
  ...
  if (isDismissedInSession) {
    return (
      <section data-feed-stability-anchor className="pt-4">
          ...
          <>
            {analysisItems.length > 0 ? (
              <div className="mt-2 space-y-3">
                ...
                {visibleAnalysisItems.map((item) => {
                  const rawBridge = getItemBridge(item);
                  const bridge = rawBridge.trim() === (threadSubtitle ?? '').trim() ? '' : rawBridge;
            ...
            <div className="mt-2 space-y-3">
              {visibleItems.map((item) => {
                // A bridge that repeats the thread's own subtitle verbatim is duplication, not
                // information (a real thread rendered its subtitle twice back-to-back).
                const rawBridge = getItemBridge(item);
                const bridge = rawBridge.trim() === (threadSubtitle ?? '').trim() ? '' : rawBridge;
