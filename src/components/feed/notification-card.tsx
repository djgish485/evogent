...
function normalizeNotificationBody(value: string | null | undefined): string {
...
export function NotificationCard({
  ...
  const normalizedBody = normalizeNotificationBody(item.text);
  const normalizedExcerpt = normalizeNotificationBody(item.excerpt);
  const shouldRenderExcerpt = Boolean(normalizedExcerpt && normalizedExcerpt !== normalizedBody);

  // Body tap opens the notification's source when it has one (a link to the thing it's about) —
  // the same "tap the card, see the thing" rule every other card follows.
  const notifSourceUrl = (() => {
    const u = item.url?.trim();
    if (u && /^https?:\/\//i.test(u)) return u;
    const s = typeof item.metadata?.sourceUrl === 'string' ? item.metadata.sourceUrl.trim() : '';
  ...
  return (
    <article
      ...
      onClick={notifSourceUrl ? (event) => {
        const target = event.target as HTMLElement | null;
        if (target?.closest('button, a, input, textarea, select, [role="button"]')) return;
        openSourceUrl(notifSourceUrl);
      } : undefined}
