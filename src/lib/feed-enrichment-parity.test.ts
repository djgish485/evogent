...
// Source-shape parity corpus is synthetic by construction. Production captures are private
// install evidence and must never be copied into this public test tree.
const parityCapturedAt = '2000-01-01T00:00:00.000Z';
...
describe('feed enrichment synthetic source-shape parity corpus', () => {
  ...
  test('replays one representative synthetic cache/feed pair per source', () => {
    const fixtures = loadParityFixtures();
    assert.equal(fixtures.length, 4);

    for (const source of fixtureSources) {
      assert.equal(fixtures.filter((fixture) => fixture.source === source).length, 1);
    }
