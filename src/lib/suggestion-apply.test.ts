...
describe('suggestion-apply', () => {
  ...
  test('applyUnifiedDiff uses hunk line numbers for markdown bullet replacements', () => {
    const content = [
      ...
      '## Per-Cycle Volume & Balance',
      '',
      '- **4-5 items on Topic Alpha** — breaking developments',
    ...
    const diff = [
      ...
      '@@ -5,1 +5,1 @@',
      '- **4-5 items on Topic Alpha** — breaking developments',
      '+- **5-6 items on Topic Alpha** — breaking developments',
    ].join('\n');

    const result = applyUnifiedDiff(content, diff);

    assert.strictEqual(result.changed, true);
    assert.ok(result.content.includes('- **5-6 items on Topic Alpha** — breaking developments'));
    assert.ok(!result.content.includes('- **4-5 items on Topic Alpha** — breaking developments'));
  ...
  test('replaceSectionContent replaces section body instead of appending list entries', () => {
    ...
    assert.ok(!result.content.includes('- **8-10 items on Topic Alpha**'));
  });
