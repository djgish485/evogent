...
export function selectEmbeddingBackend(
  ...
  configuredBackend = process.env.EVOGENT_EMBEDDING_BACKEND,
): 'local-model' | 'hash' {
  // onnxruntime-node does not publish Android binaries. The maintained model package is an
  // optional host capability; Android uses the deterministic, dependency-free fallback.
  if (platform === 'android' || configuredBackend === 'hash') return 'hash';
  return 'local-model';
}
...
async function getEmbeddingPipeline(): Promise<FeatureExtractionPipeline> {
  ...
  pipelineLoading = (async () => {
    ...
    console.error('[embeddings] Loading Xenova/all-MiniLM-L6-v2 (first run downloads model)...');
    const startedAt = Date.now();
    const loaded = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2');
    const elapsedMs = Date.now() - startedAt;
    console.error(`[embeddings] Model ready in ${elapsedMs}ms`);
...
export async function generateEmbedding(text: string): Promise<number[]> {
  ...
  if (selectEmbeddingBackend() === 'hash') {
