...
export function buildBatchEnrichmentPrompt(posts: FeedItem[], options: { requestId?: string } = {}): string {
  ...
  return [
    ...
    '- HARD CAP: at most 2 reply/comment children per item across all sources. When more than 2 qualify, keep only the 2 strongest - a sharp counterpoint, a from-the-source correction, or decisive evidence beats everything else - and record the surplus honestly in replyAudit (visibleReplyCount vs savedReplyCount).',
