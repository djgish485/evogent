...
describe('ensureFeedSchema', () => {
  test('creates all required tables', () => {
    withSchemaDb((db) => {
      ...
      assert.ok(tables.has('feed_engagement_sessions'));
      ...
      assert.ok(tables.has('config_apply_tasks'));
      ...
      assert.ok(tables.has('curation_bench'));

      const engagementColumns = new Set(
        (db.prepare(`PRAGMA table_info(feed_engagement_sessions)`).all() as Array<{ name: string }>)
          .map((row) => row.name),
      );
      assert.ok(engagementColumns.has('user_scrolled'));

      const benchColumns = new Set(
        (db.prepare(`PRAGMA table_info(curation_bench)`).all() as Array<{ name: string }>)
          .map((row) => row.name),
      );
  ...
  test('adds user scroll evidence to a legacy engagement ledger', () => {
    ...
    try {
      db.exec(`
        CREATE TABLE feed_engagement_sessions (
        ...
        INSERT INTO feed_engagement_sessions (session_id, feed_item_id)
        VALUES ('detail:legacy:session', 'legacy-item');
      `);
      ...
      assert.deepStrictEqual(
        db.prepare(`
          SELECT user_scrolled
          FROM feed_engagement_sessions
          WHERE session_id = 'detail:legacy:session'
        `).get(),
        { user_scrolled: 0 },
      );
    } finally {
  ...
  test('adds quarantine receipts to a legacy curation bench', () => {
    const db = new Database(':memory:');
    try {
      db.exec(`
        CREATE TABLE curation_bench (
      ...
      assert.ok(columns.has('quarantined_at_ms'));
  ...
  test('schema cleanup removes only orphaned replay-only chat sessions', () => {
    withSchemaDb((db) => {
      db.exec(`
        INSERT INTO chat_sessions (
          ...
          ('11111111-1111-4111-8111-111111111111', 'claude', '11111111-1111-4111-8111-111111111111', '11111111-1111-4111-8111-111111111111', 'Replay Only', '2026-04-26 09:50:03', '2026-04-26 09:50:03'),
          ('22222222-2222-4222-8222-222222222222', 'claude', '22222222-2222-4222-8222-222222222222', '22222222-2222-4222-8222-222222222222', 'User Backed', '2026-04-26 09:50:03', '2026-04-26 09:50:03'),
          ('33333333-3333-4333-8333-333333333333', 'claude', '33333333-3333-4333-8333-333333333333', '33333333-3333-4333-8333-333333333333', 'Feed Backed', '2026-04-26 09:50:03', '2026-04-26 09:50:03');
        ...
        INSERT INTO chat_messages (
          ...
          ('chat-replay-only', 'chat', 'agent', 'task-replay-only', '11111111-1111-4111-8111-111111111111', 'audit replay', '2026-04-26T09:50:03.000Z', 'delivered', '{"source":"chat-output.jsonl"}'),
          ('chat-user-backed-agent', 'chat', 'agent', 'task-user-backed', '22222222-2222-4222-8222-222222222222', 'real reply', '2026-04-26T09:50:03.000Z', 'delivered', '{"source":"chat-output.jsonl"}'),
          ('msg-user-backed', 'chat', 'user', NULL, '22222222-2222-4222-8222-222222222222', 'real user message', '2026-04-26T09:49:30.000Z', 'delivered', NULL),
          ('chat-feed-backed', 'chat', 'agent', 'task-feed-backed', '33333333-3333-4333-8333-333333333333', 'feed-backed reply', '2026-04-26T09:50:03.000Z', 'delivered', '{"source":"chat-output.jsonl"}');

        INSERT INTO feed (
          id, type, source, origin_session_id, text, published_at
        ) VALUES (
          'feed-backed-row', 'analysis', 'unit-test', '33333333-3333-4333-8333-333333333333', 'feed-backed', '2026-04-26T09:51:00.000Z'
        );
      `);
  ...
  test('creates expected indexes', () => {
    withSchemaDb((db) => {
      ...
      const expected = [
        ...
        'feed_engagement_sessions_item_opened_idx',
        'feed_engagement_sessions_opened_idx',
  ...
  test('repairs structurally misclassified Twitter article rows regardless of creation window', () => {
    withSchemaDb((db) => {
      db.exec(`
        INSERT INTO feed (
          ...
          (
            'legacy-twitter-article-a',
            'article',
            'twitter',
            '2030455675357143260',
            'Untitled article',
            'tweet-shaped article',
          ...
          (
            'legacy-twitter-article-b',
            'article',
            'twitter',
            '2030455675357143261',
            'Untitled article',
            'tweet-shaped article from another import',
          ...
          (
            'legacy-twitter-article-c',
            'article',
            'twitter',
            '2030455675357143262',
            'Third misclassified row',
            'same invalid shape at another time',
      ...
      for (const id of ['legacy-twitter-article-a', 'legacy-twitter-article-b', 'legacy-twitter-article-c']) {
        ...
        assert.deepStrictEqual(metadata.twitterCanonicalization, {
          ...
          originalUrl: id === 'legacy-twitter-article-a'
            ? 'https://x.com/example/status/2030455675357143260'
            : id === 'legacy-twitter-article-b'
              ? 'https://twitter.com/example/status/2030455675357143261'
              : 'https://x.com/example/status/2030455675357143262',
  ...
  test('repairs article publish times only from source metadata', () => {
    withSchemaDb((db) => {
      db.prepare(`
        INSERT INTO feed (id, type, source, source_id, text, metadata, published_at, created_at)
        VALUES
          (
            'metadata-backed-article',
            'article',
            'publisher',
            'https://news.example.com/article/metadata-backed',
            'Metadata-backed article',
            '{"article":{"datePublished":"2000-01-02T03:04:05Z"}}',
            '2000-01-03T03:04:05.000Z',
            '2000-01-03T03:04:06.000Z'
          ),
          (
            'article-without-source-date',
            'article',
            'publisher',
            'https://news.example.com/article/no-source-date',
            'Article without source date',
            '{}',
            '2000-01-04T03:04:05.000Z',
            '2000-01-04T03:04:06.000Z'
          )
      `).run();
      ...
      const metadataBacked = rowsById.get('metadata-backed-article');
      const withoutSourceDate = rowsById.get('article-without-source-date');

      assert.strictEqual(metadataBacked?.published_at, '2000-01-02T03:04:05.000Z');
      assert.strictEqual(metadataBacked?.published_at_ms, Date.parse('2000-01-02T03:04:05.000Z'));
      const metadataBackedJson = JSON.parse(metadataBacked?.metadata ?? '{}') as Record<string, unknown>;
      assert.deepStrictEqual(
        (metadataBackedJson.publishEvidence as Record<string, unknown>)?.publishedAt,
        '2000-01-02T03:04:05.000Z',
      );

      assert.strictEqual(withoutSourceDate?.published_at, '2000-01-04T03:04:05.000Z');
      assert.strictEqual(withoutSourceDate?.published_at_ms, Date.parse('2000-01-04T03:04:05.000Z'));
      const withoutSourceDateJson = JSON.parse(withoutSourceDate?.metadata ?? '{}') as Record<string, unknown>;
      assert.strictEqual(withoutSourceDateJson.publishEvidence, undefined);
    });
  });
  ...
  test('preserves config suggestions that are bound to authoritative apply tasks', () => {
    withSchemaDb((db) => {
      db.prepare(`
        INSERT INTO feed (id, type, text, published_at, metadata)
        VALUES (?, 'suggestion', ?, ?, ?)
      `).run(
        'config-authoritative-1',
        'Raise usage level.',
        ...
        JSON.stringify({
          ...
          taskId: 'config-task-authoritative-1',
          configField: 'Usage Level',
          configFile: 'data/config.md',
          proposedValue: 'High',
        }),
      ...
      db.prepare(`
        ...
        VALUES (?, ?, 'config', 'Usage Level', 'High', ?, 'queued')
      `).run(
        'config-task-authoritative-1',
        'config-authoritative-1',
        'a'.repeat(64),
      );
      ...
      const suggestion = db.prepare(`
        SELECT id
        FROM feed
        WHERE id = 'config-authoritative-1'
      `).get() as { id: string } | undefined;
      const task = db.prepare(`
        SELECT task_id, suggestion_id, status
        FROM config_apply_tasks
        WHERE task_id = 'config-task-authoritative-1'
      `).get() as {
        task_id: string;
      ...
      assert.deepStrictEqual(suggestion, { id: 'config-authoritative-1' });
      assert.deepStrictEqual(task, {
        task_id: 'config-task-authoritative-1',
        suggestion_id: 'config-authoritative-1',
        status: 'queued',
      });
    });
  });
  ...
  test('deduplicates legacy feed rows before adding the source_id unique index', () => {
    ...
    try {
      ...
      assert.strictEqual(configApplyTable?.name, 'config_apply_tasks');
  ...
  test('merges structurally equivalent article source-id pairs while keeping the older rows', () => {
    const db = new Database(':memory:');
    ...
    try {
      ...
      db.prepare(`
        ...
        VALUES
          ('legacy-first', 'article', 'newsletter', 'journal.example.com:/p/synthetic-one', 'Synthetic Essay One', 'older legacy row', 'https://journal.example.com/p/synthetic-one', '2026-03-01T00:00:00.000Z', '2026-03-01T00:01:00.000Z'),
          ('url-first', 'article', 'newsletter', 'https://journal.example.com/p/synthetic-one', 'Synthetic Essay One', 'newer canonical row', 'https://journal.example.com/p/synthetic-one', '2026-03-01T00:00:00.000Z', '2026-03-01T00:02:00.000Z'),
          ('legacy-second', 'article', 'newsletter', 'example-letter:/p/synthetic-two', 'Synthetic Essay Two', 'older legacy row', 'https://example-letter.substack.com/p/synthetic-two', '2026-03-02T00:00:00.000Z', '2026-03-02T00:01:00.000Z'),
          ('url-second', 'article', 'newsletter', 'https://example-letter.substack.com/p/synthetic-two', 'Synthetic Essay Two', 'newer canonical row', 'https://example-letter.substack.com/p/synthetic-two', '2026-03-02T00:00:00.000Z', '2026-03-02T00:02:00.000Z'),
          ('legacy-third', 'article', 'newsletter', 'research.example.org:/p/synthetic-three', 'Synthetic Essay Three', 'older legacy row', 'https://research.example.org/p/synthetic-three', '2026-03-03T00:00:00.000Z', '2026-03-03T00:01:00.000Z'),
          ('url-third', 'article', 'newsletter', 'https://research.example.org/p/synthetic-three', 'Synthetic Essay Three', 'newer canonical row', 'https://research.example.org/p/synthetic-three', '2026-03-03T00:00:00.000Z', '2026-03-03T00:02:00.000Z'),
          ('child-row', 'analysis', 'claude', 'child-row', 'Child', 'child row', NULL, '2026-03-03T00:03:00.000Z', '2026-03-03T00:03:30.000Z')
      `).run();

      db.prepare(`
        INSERT INTO interactions (feed_item_id, action)
        VALUES ('url-third', 'like')
      `).run();

      db.prepare(`
        INSERT INTO preferences (id, signal_type, source, text, feed_item_id)
        VALUES ('pref-third', 'liked', 'app_thumbsup', 'pref row', 'url-third')
      `).run();

      db.prepare(`
        INSERT INTO code_fix_tasks (suggestion_id, task_id, status)
        VALUES ('url-third', 'task-1', 'running')
      `).run();

      db.prepare(`
        UPDATE feed
        SET parent_id = 'url-third'
        WHERE id = 'child-row'
      `).run();
      ...
      const rows = db.prepare(`
        SELECT id, source_id
        FROM feed
        WHERE title IN ('Synthetic Essay One', 'Synthetic Essay Two', 'Synthetic Essay Three')
        ORDER BY id
      `).all() as Array<{ id: string; source_id: string }>;

      assert.deepStrictEqual(rows, [
        { id: 'legacy-first', source_id: 'journal.example.com:/p/synthetic-one' },
        { id: 'legacy-second', source_id: 'example-letter:/p/synthetic-two' },
        { id: 'legacy-third', source_id: 'research.example.org:/p/synthetic-three' },
      ]);
      ...
      const childRow = db.prepare(`
        ...
        WHERE id = 'child-row'
      `).get() as { parent_id: string | null } | undefined;
      assert.strictEqual(childRow?.parent_id, 'legacy-third');
      ...
      const interactionRow = db.prepare(`
        ...
        WHERE action = 'like'
      `).get() as { feed_item_id: string; action: string } | undefined;
      assert.deepStrictEqual(interactionRow, { feed_item_id: 'legacy-third', action: 'like' });
      ...
      const preferenceRow = db.prepare(`
        ...
        WHERE id = 'pref-third'
      `).get() as { feed_item_id: string | null } | undefined;
      assert.strictEqual(preferenceRow?.feed_item_id, 'legacy-third');
      ...
      const codeFixRow = db.prepare(`
        ...
        WHERE task_id = 'task-1'
      `).get() as { suggestion_id: string; task_id: string } | undefined;
      assert.deepStrictEqual(codeFixRow, { suggestion_id: 'legacy-third', task_id: 'task-1' });
    } finally {
      db.close();
    }
  });
