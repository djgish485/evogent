...
import { getThreadDisplayGroupIdentity } from '@/lib/feed-normalize';
import { parseThreadFilterIds } from '@/lib/thread-display';
...
export interface FeedArrangementRunInput {
  source?: string | null;
  ordering: FeedArrangeOrderingInput[];
  threads: FeedArrangeThreadInput[];
  updatedItemIds?: string[];
  carryForwardAudit?: unknown;
  curatorCarryForwardAudit?: unknown;
}
...
export const PRIMARY_FEED_SLATE_MAX_ITEMS = 50;
export const PRIMARY_FEED_CONTENT_TYPES = ['tweet', 'article', 'analysis'] as const;
export const LEGACY_CARRY_FORWARD_THREAD_ID = 'carry-forward-unread';
const SINGLETON_SHIPMENT_THREAD_PREFIX = 'shipment-singleton:';
const LEGACY_SINGLETON_SHIPMENT_THREAD_PREFIX = 'carry-forward-singleton:';

/**
 * A standalone primary item still needs a stable shipment identity for arrange/cap
 * mechanics, but it is not a display thread. Its feed-item id is already durable
 * and globally unique, so deriving the shipment id from it keeps repeated arrange
 * passes stable without welding unrelated singletons into one lane.
 */
export function getSingletonShipmentThreadId(feedItemId: string): string {
  const normalizedFeedItemId = feedItemId.trim();
  if (!normalizedFeedItemId) {
    throw new Error('singleton shipment requires a feed item id');
  }
  return `${SINGLETON_SHIPMENT_THREAD_PREFIX}${normalizedFeedItemId}`;
}

export function isSingletonShipmentThreadId(
  threadId: string | null | undefined,
): boolean {
  const normalizedThreadId = threadId?.trim() ?? '';
  return (
    normalizedThreadId.startsWith(SINGLETON_SHIPMENT_THREAD_PREFIX)
    && normalizedThreadId.length > SINGLETON_SHIPMENT_THREAD_PREFIX.length
  ) || (
    normalizedThreadId.startsWith(LEGACY_SINGLETON_SHIPMENT_THREAD_PREFIX)
    && normalizedThreadId.length > LEGACY_SINGLETON_SHIPMENT_THREAD_PREFIX.length
  );
}
...
function getContiguousDisplayThreadFeedPageItems(
  items: FeedItem[],
  offset: number,
  limit: number,
): FeedItem[] {
  const start = Math.max(0, offset);
  let end = Math.min(items.length, start + Math.max(0, limit));
  if (start >= end) {
    return [];
  }

  // Arrange is the sole owner of thread shipments. Pagination only prevents a
  // contiguous, explicitly displayed shipment from being split at the page
  // boundary; it never scans ahead for reused ids or reconstructs a grouping.
  const boundaryIdentity = getThreadDisplayGroupIdentity(items[end - 1]);
  if (!boundaryIdentity) {
  ...
  while (
    ...
    && getThreadDisplayGroupIdentity(items[end])?.key === boundaryIdentity.key
  ) {
    end += 1;
  }
...
function normalizeFlatThreadMetadata(raw: Record<string, unknown>): FeedThreadMetadata | null {
  const threadId = typeof raw.threadId === 'string' ? raw.threadId.trim() : '';
  if (!threadId) return null;

  const thread: FeedThreadMetadata = { threadId };
  ...
  const threadProminence = normalizeFeedProminence(raw.threadProminence);
  if (threadProminence) {
    thread.prominence = threadProminence;
  }
...
function normalizeMetadata(input: unknown): FeedMetadata | null {
  ...
  const thread = normalizeThreadMetadata(raw.thread) ?? normalizeFlatThreadMetadata(raw);
  if (thread) {
...
function rowToFeedItem(
  row: FeedRow,
  options: { liveArrangementActive?: boolean } = {},
): FeedItem {
  const metadata = normalizeMetadata(parseJsonRecord(row.metadata));
  const shouldShowThread = row.display_thread_active !== 0;
  const hasCurrentDisplayOrder = typeof row.display_order === 'number' && Number.isFinite(row.display_order);
  const threadDisplayEnabled = options.liveArrangementActive ? hasCurrentDisplayOrder : true;
  const metadataThread = metadata?.thread?.threadId?.trim() ? metadata.thread : null;
  const metadataThreadId = metadataThread?.threadId.trim() || null;
  const metadataThreadTitle = metadataThread?.threadTitle?.trim() || null;
  const metadataThreadSubtitle = metadataThread?.threadRationale?.trim() || null;
  const arrangedThreadId = typeof row.thread_id === 'string' && row.thread_id.trim()
    ? row.thread_id.trim()
    : null;
  // Singleton shipment ids are internal arrangement boundaries, not visible editorial
  // threads. Suppress both the synthetic id and the item's old metadata thread so a lone
  // carry-forward renders as a normal card without resurrecting a one-item banner.
  const suppressSingletonShipmentThread = isSingletonShipmentThreadId(arrangedThreadId);
  const allowMetadataThreadFallback = !suppressSingletonShipmentThread
    && (!options.liveArrangementActive || Boolean(arrangedThreadId));
  const metadataBridge = typeof metadata?.bridge === 'string' && metadata.bridge.trim()
    ? metadata.bridge.trim()
    : null;
  const threadId = threadDisplayEnabled && !suppressSingletonShipmentThread
    ? (shouldShowThread
      ? arrangedThreadId ?? (allowMetadataThreadFallback ? metadataThreadId : null)
      : (allowMetadataThreadFallback ? metadataThreadId : null))
    : null;
  const threadTitle = threadDisplayEnabled && !suppressSingletonShipmentThread
    ? (shouldShowThread
      ? row.display_thread_title ?? (allowMetadataThreadFallback ? metadataThreadTitle : null)
      : (allowMetadataThreadFallback ? metadataThreadTitle : null))
    : null;
  const threadSubtitle = threadDisplayEnabled && !suppressSingletonShipmentThread
    ? (shouldShowThread
      ? row.display_thread_subtitle ?? (allowMetadataThreadFallback ? metadataThreadSubtitle : null)
      : (allowMetadataThreadFallback ? metadataThreadSubtitle : null))
    : null;
  ...
  return {
    ...
    title: row.title,
    text: row.text,
    // Keep generated display reasons product-generic; private taste stays in private metadata.
    url: row.url ?? (typeof metadata?.sourceUrl === 'string' && /^https?:\/\//.test(metadata.sourceUrl)
      ? metadata.sourceUrl
      : null),
    excerpt: row.excerpt,
    ...
    displayOrder: hasCurrentDisplayOrder ? row.display_order : null,
    threadDisplayEnabled,
    threadId,
    displaySubtitle: row.display_subtitle ?? metadataBridge,
    threadTitle,
    threadSubtitle,
    metrics: {
...
export function normalizeFeedInput(input: unknown): FeedInsertInput | null {
  ...
  const normalized: FeedInsertInput = {
    ...
    authorDisplayName,
    authorAvatarUrl,
    reason: normalizeFeedReason(raw, metadata),
...
function setFeedItemInteraction(feedItemId: string, action: string, enabled: boolean): void {
  ...
  if (enabled) {
    ...
    if (action === 'dislike') {
      // Vacate the display slot immediately. The display query hides disliked items, but a row
      // that keeps its display_order until the next arrange makes every raw-DB reader (audits,
      // intent checks) see a top-of-feed the user doesn't, and the rearrange backstop rebuilds
      // its pool from display_order. Undo does NOT restore the slot — the next arrange re-seats
      // the item on merit.
      db.prepare('UPDATE feed SET display_order = NULL WHERE id = ?').run(feedItemId);
    }
    return;
  }
...
export function listTopLevelItemsWithIncompleteEnrichment(limit = 30): FeedItem[] {
  ...
  return rows.map((row) => rowToFeedItem(row));
}
...
function clearChildFeedDisplayMetadata(db: ReturnType<typeof getDb>): void {
  db.prepare(`
    UPDATE feed
    SET
      display_order = NULL,
    ...
    WHERE parent_id IS NOT NULL
      AND (
        ...
        OR display_subtitle IS NOT NULL
      )
  `).run();
}
...
export function getLastArrangeAtMs(): number | null {
  const db = getDb();
  const row = db.prepare(`
    SELECT MAX(arranged_at_ms) AS last_arrange_at_ms
    FROM (
      SELECT updated_at_ms AS arranged_at_ms
      FROM feed_threads
      UNION ALL
      SELECT created_at_ms AS arranged_at_ms
      FROM feed_arrangement_runs
    )
  `).get() as { last_arrange_at_ms: number | null } | undefined;

  return typeof row?.last_arrange_at_ms === 'number' ? row.last_arrange_at_ms : null;
}

export interface PendingSuggestionPlacement {
  id: string;
...
/**
 * Every arrange pass places pending suggestions deterministically (important ones above all
 * content, code fixes at the back), so it needs the pending set plus the full suggestion id
 * set to strip whatever ordering the curator may have given them.
 */
export function getSuggestionArrangePlacements(): {
  pending: PendingSuggestionPlacement[];
  allSuggestionIds: Set<string>;
} {
  const db = getDb();
  const rows = db.prepare(`
    SELECT id, metadata, created_at_ms
    FROM feed
    WHERE type = 'suggestion'
      AND parent_id IS NULL
  `).all() as Array<{ id: string; metadata: string | null; created_at_ms: number }>;

  const pending: PendingSuggestionPlacement[] = [];
  const allSuggestionIds = new Set<string>();
  for (const row of rows) {
    allSuggestionIds.add(row.id);
    ...
    const status = typeof metadata.suggestionStatus === 'string'
      ? metadata.suggestionStatus.trim().toLowerCase()
      : '';
    // A card stays ON THE GLASS through its whole execution lifecycle:
    // 'dispatched'/'running' must keep rendering "Agent dispatched…",
    // and a completed/failed execution must render its result — the pending-only filter
    // silently unlisted those on the next arrange (display_order nulled), so the approve
    // button appeared to do nothing. The execution-result endpoint exists to prevent this.
    // Plain accepted/dismissed/handled with no result stays
    // excluded as before, and a rendered result ages off the glass after 48h (it remains
    // in the Suggestions view).
    const hasExecutionResult = typeof metadata.executionResult === 'string'
      && metadata.executionResult.trim() !== '';
    const activeLifecycle = status === 'dispatched' || status === 'running'
      || ((status === 'accepted' || status === 'failed') && hasExecutionResult);
    if (status && status !== 'pending' && !activeLifecycle) continue;
    const resultAtMsRaw = Number(metadata.executionResultAtMs);
    if (hasExecutionResult && Number.isFinite(resultAtMsRaw) && resultAtMsRaw > 0
      && Date.now() - resultAtMsRaw > 48 * 60 * 60 * 1000) continue;
    const suggestionType = typeof metadata.suggestionType === 'string'
      ? metadata.suggestionType.trim().toLowerCase()
      : '';
    const importanceRaw = typeof metadata.importance === 'string'
      ? metadata.importance.trim().toLowerCase()
      : '';
    const expiresAtMsRaw = Number(metadata.expiresAtMs);
    pending.push({
      id: row.id,
      suggestionType,
      importance: importanceRaw === 'high' || importanceRaw === 'urgent' ? 'high' : 'normal',
      createdAtMs: row.created_at_ms,
      expiresAtMs: Number.isFinite(expiresAtMsRaw) && expiresAtMsRaw > 0 ? expiresAtMsRaw : null,
    });
  }
  pending.sort((left, right) => right.createdAtMs - left.createdAtMs);
  return { pending, allSuggestionIds };
}

export function getQuarantinedFeedItemIds(ids: string[]): Set<string> {
  const quarantinedIds = new Set<string>();
  if (!ids.length) return quarantinedIds;
  ...
  for (let i = 0; i < ids.length; i += 400) {
    ...
    for (const row of rows) quarantinedIds.add(row.id);
  }

  return quarantinedIds;
}

export function getCurrentFeedArrangeInput(): {
  ordering: FeedArrangeOrderingInput[];
  threads: FeedArrangeThreadInput[];
} {
  const db = getDb();
  const rows = db.prepare(`
    SELECT id, display_order, thread_id, display_subtitle, metadata
    FROM feed
    WHERE display_order IS NOT NULL
      AND parent_id IS NULL
    ORDER BY display_order ASC, created_at_ms DESC
  `).all() as Array<{
    id: string;
  ...
  // Reconstruct the original editorial identity before the next arrange. A real
  // thread survives only while 2+ displayed members share it. Every other row
  // receives its own stable shipment id, including rows from the retired shared
  // carry-forward lane and stale one-item metadata threads.
  const metadataThreads = new Map<string, { title: string; subtitle: string | null }>();
  const metadataThreadIdByItem = new Map<string, string>();
  for (const row of rows) {
    if (!row.metadata) continue;
    try {
      const meta = JSON.parse(row.metadata) as { thread?: { threadId?: unknown; threadTitle?: unknown; threadRationale?: unknown } };
      const t = meta.thread;
      const tid = typeof t?.threadId === 'string' ? t.threadId.trim() : '';
      if (!tid) continue;
      metadataThreadIdByItem.set(row.id, tid);
      if (!metadataThreads.has(tid)) {
        metadataThreads.set(tid, {
          title: typeof t?.threadTitle === 'string' && t.threadTitle.trim() ? t.threadTitle.trim() : tid,
          subtitle: typeof t?.threadRationale === 'string' && t.threadRationale.trim() ? t.threadRationale.trim() : null,
        });
      }
    } catch { /* malformed metadata — skip */ }
  }

  const originalThreadIdByItem = new Map<string, string>();
  const originalThreadMemberCount = new Map<string, number>();
  for (const row of rows) {
    const persistedThreadId = row.thread_id?.trim() ?? '';
    const metadataThreadId = metadataThreadIdByItem.get(row.id)?.trim() ?? '';
    const originalThreadId = persistedThreadId
      && persistedThreadId !== LEGACY_CARRY_FORWARD_THREAD_ID
      && !isSingletonShipmentThreadId(persistedThreadId)
      && activeThreadIds.has(persistedThreadId)
      ? persistedThreadId
      : metadataThreadId
        && metadataThreadId !== LEGACY_CARRY_FORWARD_THREAD_ID
        && !isSingletonShipmentThreadId(metadataThreadId)
        ? metadataThreadId
        : '';
    if (!originalThreadId) continue;
    originalThreadIdByItem.set(row.id, originalThreadId);
    originalThreadMemberCount.set(
      originalThreadId,
      (originalThreadMemberCount.get(originalThreadId) ?? 0) + 1,
    );
  }

  const ordering: FeedArrangeOrderingInput[] = rows.map((row, index) => {
    const originalThreadId = originalThreadIdByItem.get(row.id) ?? null;
    const shipmentThreadId = originalThreadId
      && (originalThreadMemberCount.get(originalThreadId) ?? 0) >= 2
      ? originalThreadId
      : getSingletonShipmentThreadId(row.id);

    return {
      feedItemId: row.id,
      displayOrder: index + 1,
      threadId: shipmentThreadId,
      displaySubtitle: row.display_subtitle,
    };
  });

  const referencedThreadIds = new Set(
    ordering.map((entry) => entry.threadId).filter((id): id is string => Boolean(id)),
  );
  const threads: FeedArrangeThreadInput[] = activeThreads
    .filter((thread) => (
  ...
  // Add the metadata-derived fresh threads that aren't already registered.
  for (const [tid, info] of metadataThreads) {
    if (!referencedThreadIds.has(tid)) continue;
    if (isSingletonShipmentThreadId(tid)) continue;
    if (threads.some((thread) => thread.id === tid)) continue;
    threads.push({ id: tid, title: info.title, subtitle: info.subtitle, active: true });
  }

  return { ordering, threads };
}
...
export function arrangeFeedDisplay(input: {
  ...
  const quarantinedIds = getQuarantinedFeedItemIds(
    input.ordering.map((entry) => entry.feedItemId),
  );
  if (quarantinedIds.size > 0) {
    input = {
      ordering: input.ordering.filter((entry) => !quarantinedIds.has(entry.feedItemId)),
  ...
  // Disliked items never receive a display slot, no matter who is calling (curator arrange,
  // the rearrange backstop reconstructing from the current display, a manual POST). The display
  // query already hides them, but letting them hold display_order makes raw-DB readers judge a
  // feed that is not visible and lets a rearrange re-promote them. Filter at this chokepoint
  // instead of in every caller.
  const orderingIds = input.ordering.map((entry) => entry.feedItemId);
  const dislikedIds = new Set<string>();
  for (let i = 0; i < orderingIds.length; i += 500) {
    const chunk = orderingIds.slice(i, i + 500);
    const placeholders = chunk.map(() => '?').join(', ');
    const rows = db.prepare(`
      SELECT DISTINCT feed_item_id FROM interactions
      WHERE action = 'dislike' AND feed_item_id IN (${placeholders})
    `).all(...chunk) as Array<{ feed_item_id: string }>;
    for (const row of rows) dislikedIds.add(row.feed_item_id);
  }
  if (dislikedIds.size > 0) {
    input = {
      ordering: input.ordering.filter((entry) => !dislikedIds.has(entry.feedItemId)),
      threads: input.threads,
    };
  }

  let primaryContentCount = 0;
  const arrangedIds = input.ordering.map((entry) => entry.feedItemId);
  for (let i = 0; i < arrangedIds.length; i += 500) {
    const chunk = arrangedIds.slice(i, i + 500);
    const placeholders = chunk.map(() => '?').join(', ');
    const typePlaceholders = PRIMARY_FEED_CONTENT_TYPES.map(() => '?').join(', ');
    const row = db.prepare(`
      ...
      WHERE id IN (${placeholders})
        ...
        AND parent_id IS NULL
    `).get(...chunk, ...PRIMARY_FEED_CONTENT_TYPES) as { count: number };
    primaryContentCount += row.count;
  }
  if (primaryContentCount > PRIMARY_FEED_SLATE_MAX_ITEMS) {
    throw new Error(
      `Feed arrangement exceeds the ${PRIMARY_FEED_SLATE_MAX_ITEMS}-item primary content slate contract`,
    );
  }

  const applyArrange = db.transaction(() => {
    clearChildFeedDisplayMetadata(db);
    ...
    if (input.ordering.length === 0) {
      ...
      const orderedItemIds = input.ordering.map((item) => item.feedItemId);
      const placeholders = orderedItemIds.map(() => '?').join(', ');
      db.prepare(`
        UPDATE feed
        SET
          display_order = NULL,
...
export function recordFeedArrangementRun(input: FeedArrangementRunInput): number {
  const db = getDb();
  const source = trimToNullableText(input.source) ?? 'curator';
  return db.transaction(() => {
    ...
    `).run({
      source,
      ordering_count: input.ordering.length,
      thread_count: input.threads.length,
      updated_item_count: input.updatedItemIds?.length ?? 0,
      carry_forward_audit: safeJsonStringify(input.carryForwardAudit),
      curator_carry_forward_audit: safeJsonStringify(input.curatorCarryForwardAudit),
      ordering_snapshot: safeJsonStringify(input.ordering),
      thread_snapshot: safeJsonStringify(input.threads),
    });
...
export function arrangeFeedBackstopForCycle(startedAtMs: number): {
  fired: boolean;
  itemCount: number;
  reason: string;
} {
  if (!Number.isFinite(startedAtMs) || startedAtMs <= 0) {
    return { fired: false, itemCount: 0, reason: 'invalid_started_at_ms' };
  }

  const db = getDb();
  clearChildFeedDisplayMetadata(db);

  const hasPersistedArrangement = Boolean(db.prepare(`
    SELECT 1 AS present
    FROM feed
    WHERE parent_id IS NULL AND display_order IS NOT NULL
    LIMIT 1
  `).get());
  return hasPersistedArrangement
    ? { fired: false, itemCount: 0, reason: 'curator_arrangement_authoritative' }
    : { fired: false, itemCount: 0, reason: 'no_curator_arrangement' };
}
...
function buildFeedSearchExpression(alias: string): string {
  return `lower(trim(
    ...
    coalesce(${alias}.tags, '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.bridge'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.currentInterestReason'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.thread.threadTitle'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.thread.threadRationale'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.threadTitle'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.threadRationale'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.article.title'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.article.description'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.linkCard.title'), '') || ' ' ||
    coalesce(json_extract(${alias}.metadata, '$.linkCard.description'), '')
  ))`;
}

function buildFeedSearchTokenClause(searchExpr: string, token: string): { sql: string; value: string } {
  if (/^[a-z0-9]+$/.test(token)) {
    return {
      sql: `(' ' || ${searchExpr} || ' ') GLOB ?`,
      value: `*[^a-z0-9]${token}[^a-z0-9]*`,
    };
  }

  return {
    sql: `${searchExpr} LIKE ? ESCAPE '\\'`,
    value: `%${escapeSqlLikePattern(token)}%`,
  };
}
...
function buildFeedListWhereClause(
  ...
  if (effectiveTypes.length === 1 && effectiveTypes[0] === 'analysis') {
    // The Analysis tab is editorial analysis only. Reflection rows and the
    // deleted May-2026 transparency/cycle-status cards are process notices
    // (they surface under Notification, not here); excluding them server-side
    // keeps the tab's first page from rendering empty after client suppression.
    whereClauses.push(`
      f.id NOT LIKE 'reflection-%'
      AND COALESCE(json_extract(f.metadata, '$.mode'), '') != 'reflection'
      AND COALESCE(json_extract(f.metadata, '$.reflectionCycle'), 0) != 1
      AND COALESCE(
        f.thread_id,
        json_extract(f.metadata, '$.thread.threadId'),
        json_extract(f.metadata, '$.threadId'),
        ''
  ...
  const normalizedThreadIds = parseThreadFilterIds(query.threadId);
  if (normalizedThreadIds.length > 0) {
    const threadPlaceholders = normalizedThreadIds.map(() => '?').join(', ');
    whereClauses.push(`(
      f.thread_id IN (${threadPlaceholders})
      OR TRIM(json_extract(f.metadata, '$.thread.threadId')) IN (${threadPlaceholders})
      OR TRIM(json_extract(f.metadata, '$.threadId')) IN (${threadPlaceholders})
    )`);
    values.push(...normalizedThreadIds, ...normalizedThreadIds, ...normalizedThreadIds);
  }

  if (searchTokens.length > 0) {
    const searchExpr = buildFeedSearchExpression('f');
    const searchFilters = searchTokens.map((token) => buildFeedSearchTokenClause(searchExpr, token));
    const searchClauses = searchFilters.map((filter) => filter.sql);
    whereClauses.push(`(${searchClauses.join(' OR ')})`);
    values.push(...searchFilters.map((filter) => filter.value));
  }
...
export function getFeedPage(
  ...
  const baseItems = rows.map((row) => rowToFeedItem(row, { liveArrangementActive: true }));
  const pageItems = getContiguousDisplayThreadFeedPageItems(
    baseItems,
    query.offset,
    query.limit,
  );
...
export function getSuggestionFeedGroup(
  ...
  const items = buildSuggestionGroupItems(
    hydrateFeedItemsForList(
      rows.map((row) => rowToFeedItem(row, { liveArrangementActive: true })),
    ),
    query.sort,
  );
