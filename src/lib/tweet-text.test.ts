...
test('stripLinkPreviewUrlsForDisplay removes previewed URLs and preserves other links', () => {
  const item: Pick<FeedItem, 'type' | 'text' | 'metadata'> = {
    ...
    text: [
      'Regional update',
      'https://news.example.com/world/region/negotiation-update/',
      'Docs https://docs.example.com/report',
    ].join('\n'),
    metadata: {
      linkPreviews: [
        {
          url: 'https://news.example.com/world/region/negotiation-update/',
      ...
      urlEntities: [
        {
          ...
          expandedUrl: 'https://news.example.com/world/region/negotiation-update/',
        },
      ],
    },
  };
...
test('stripLeadingReplyMentions removes a single leading mention', () => {
  const input = '@example_author The report includes new evidence';
  const output = stripLeadingReplyMentions(input);
  assert.equal(output, 'The report includes new evidence');
});
