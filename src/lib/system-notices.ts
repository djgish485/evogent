...
export function isLegacyTransparencyCard(item: FeedItem): boolean {
  // Legacy "transparency"/cycle-status/readout cards can outlive the mandate
  // that produced them because old rows persist in restored databases.
  const threadId = item.threadId
    ?? (typeof item.metadata?.thread?.threadId === 'string' ? item.metadata.thread.threadId : null)
    ?? (typeof item.metadata?.threadId === 'string' ? item.metadata.threadId : null);
  if (typeof threadId !== 'string') {
    return false;
  }
  const normalized = threadId.toLowerCase();
  return normalized.includes('transparency') || normalized.startsWith('curation-readout-');
}
