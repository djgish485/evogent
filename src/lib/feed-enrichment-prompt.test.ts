...
describe('feed enrichment prompt', () => {
  ...
  test('lightweight mode only captures tweet-page context and avoids feed-flooding work', () => {
    ...
    assert.match(prompt, /"linkCard":\{"type":"article","url":"https:\/\/code\.example\.com\/example\/project"/);
  ...
  test('batch enrichment resolves HN discussion URLs from sourceId when metadata omits hnUrl', () => {
    ...
    assert.match(prompt, /https:\/\/news\.ycombinator\.com\/item\?id=100001/);
    assert.doesNotMatch(prompt, /https:\/\/news\.example\.com\/research\/synthetic-systems-paper/);
  });

  test('batch prompt requires author_avatar_url before the next item', () => {
    const prompt = buildBatchEnrichmentPrompt([
      buildFeedItem({
        id: '6055f471-6dc0-4a6b-b417-7cb8f32286eb',
        url: 'https://x.com/example_author/status/1888990011223344557',
        authorAvatarUrl: null,
      }),
    ]);
  ...
  test('batch prompt treats article and link cards as supported missing facts', () => {
    ...
    assert.match(prompt, /"linkCard":\{"type":"article","url":"https:\/\/code\.example\.com\/example\/project"/);
    assert.match(prompt, /"imageAlt":"Repository preview image"/);
    assert.match(prompt, /"urlEntities":\[\{"url":"https:\/\/t\.co\/example","expandedUrl":"https:\/\/code\.example\.com\/example\/project"/);
  });
