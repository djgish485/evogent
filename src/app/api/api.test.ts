...
describe('API Integration Tests', { concurrency: false, skip: INTEGRATION_SKIP_REASON }, () => {
  ...
  describe('Chat API', () => {
    ...
    test('POST /api/chat atomically registers and echoes the exact phone scheduler cycle identity', async () => {
      const db = getDb();
      const cycleId = `phone-curation-${randomUUID()}`;
      const curatorSession = createChatSession({
        provider: 'claude',
        sessionType: 'curator',
        title: 'Receipt wiring test',
      });
      ...
      try {
        const response = await requestJson('/api/chat', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            message: '[unit] verify exact automated curation receipt wiring',
            sessionId: curatorSession.id,
            metadata: {
              trigger: 'phone_scheduler',
              curationCycleId: cycleId,
            },
          }),
        });
        assert.strictEqual(response.status, 202);
        assertObject(response.data, 'Expected phone scheduler enqueue payload');
        assert.strictEqual(response.data.curationCycleId, cycleId);
        assert.strictEqual(typeof response.data.requestId, 'string');

        const row = db.prepare(`
          SELECT request_id, triggered_by, feed_count_before
          FROM curation_log
          WHERE request_id = ?
        `).get(cycleId) as {
          request_id: string;
          triggered_by: string;
          feed_count_before: number;
        } | undefined;
        assert.ok(row);
        assert.strictEqual(row?.request_id, cycleId);
        assert.strictEqual(row?.triggered_by, 'phone_scheduler:cycle');
        assert.strictEqual(typeof row?.feed_count_before, 'number');

        const duplicate = await requestJson('/api/chat', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            message: '[unit] duplicate exact automated curation cycle identity',
            sessionId: curatorSession.id,
            metadata: {
              trigger: 'phone_scheduler',
              curationCycleId: cycleId,
        ...
        assertObject(duplicate.data, 'Expected duplicate cycle response');
        assert.strictEqual(typeof duplicate.data.message, 'string');
        assert.match(duplicate.data.message, /already been used/);
      } finally {
        db.prepare('DELETE FROM curation_log WHERE request_id = ?').run(cycleId);
      }
    });

    test('POST /api/chat rejects malformed phone scheduler cycle identities before enqueue', async () => {
      const response = await requestJson('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          message: '[unit] malformed cycle identity',
          metadata: {
            trigger: 'phone_scheduler',
            curationCycleId: '../bad',
          },
      ...
      assert.strictEqual(response.status, 400);
      assertObject(response.data, 'Expected invalid cycle response');
      assert.strictEqual(response.data.enqueued, false);
      assert.strictEqual(typeof response.data.message, 'string');
      assert.match(response.data.message, /valid curationCycleId/);
    });
    ...
    test('POST /api/internal/browse-cache/submit stores cache rows and GET /api/internal/browse-cache/items reads them back', async () => {
      ...
      const missingPublishedSourceId = `hn-missing-published-${randomUUID()}`;
      const duplicateSourceId = `hn-duplicate-${randomUUID()}`;
          ...
          items: [
            {
              ...
              authorUsername: 'example_hn_user',
              authorDisplayName: 'Example HN User',
            ...
            {
              source: 'hackernews',
              sourceId: missingPublishedSourceId,
              url: `https://example.com/${missingPublishedSourceId}`,
              title: 'Cache row without publishedAtMs',
              fetchedAtMs: completedAtMs - 1,
              expiresAtMs: completedAtMs + 60_000,
              payload: {
                sourceId: missingPublishedSourceId,
                title: 'Cache row without publishedAtMs',
            ...
            },
            {
              ...
              url: `https://example.com/${duplicateSourceId}`,
              title: 'Already accepted cached HN story',
              publishedAtMs: startedAtMs - 1_000,
              fetchedAtMs: completedAtMs - 2,
              expiresAtMs: completedAtMs + 60_000,
              payload: {
                sourceId: duplicateSourceId,
                title: 'Already accepted cached HN story',
      ...
      db.prepare(`
        ...
        VALUES (?, 'article', 'hackernews', ?, 'Already accepted cached HN story', 'already accepted', ?, '2026-06-07T20:00:00.000Z', '2026-06-07T20:01:00.000Z')
      `).run(`existing-feed-${randomUUID()}`, duplicateSourceId, `https://example.com/${duplicateSourceId}`);

      const submittableResponse = await requestJson(
        `/api/internal/browse-cache/items?source=hackernews&freshAfterMs=${completedAtMs - 1}&limit=20&requirePublishedAt=1&excludeFeedDuplicates=1`,
      );
      assert.strictEqual(submittableResponse.status, 200);
      assertObject(submittableResponse.data, 'Expected submittable browse cache items payload');
      assert.ok(Array.isArray(submittableResponse.data.items));
      const returnedIds = new Set(submittableResponse.data.items.map((item: Record<string, unknown>) => item.sourceId));
      assert.strictEqual(returnedIds.has(sourceId), true);
      assert.strictEqual(returnedIds.has(missingPublishedSourceId), false);
      assert.strictEqual(returnedIds.has(duplicateSourceId), false);
    });
  ...
  describe('Preferences API', () => {
    ...
    test('POST /api/preferences/match returns neutral nearby evidence', async () => {
      ...
      assert.ok(response.data.metric === null || response.data.metric === 'cosine' || response.data.metric === 'l2');
      assert.strictEqual(typeof response.data.evidenceCount, 'number');
      assert.ok(Array.isArray(response.data.topMatches));
      assert.strictEqual(response.data.relevanceScore, undefined);
      assert.strictEqual(response.data.verdict, undefined);
      assert.strictEqual(response.data.matchedLikes, undefined);
      assert.strictEqual(response.data.matchedDislikes, undefined);
    });
  });
                ...
                metadata: {
                  ...
                  thread: {
                    ...
                    prominence: {
                      level: 'lead',
                      source: 'homepage',
                      evidence: 'Largest publisher homepage headline.',
