...
describe('/api/feed suggestion counts', () => {
  ...
  test('returns stored prominence metadata on feed items', async () => {
    ...
    db.prepare(`
      ...
      VALUES (?, 'article', 'publisher', ?, 'Lead story', 'Lead story summary', ?, ?, ?)
    `).run(
      'prominent-article',
      'https://publisher.example.com/example',
      JSON.stringify({
        prominence: {
          level: 'lead',
          source: 'homepage',
  ...
  test('returns stored thread prominence metadata on feed items', async () => {
    ...
    db.prepare(`
      INSERT INTO feed (id, type, source, source_id, title, text, metadata, published_at, created_at)
      VALUES (?, 'article', 'publisher', ?, 'Thread story', 'Thread story summary', ?, ?, ?)
    `).run(
      'thread-prominent-article',
      'https://publisher.example.com/example-thread',
      JSON.stringify({
        cycleId: 'cycle-1',
        thread: {
          threadId: 'thread-1',
  ...
  test('returns curator display order and active live thread metadata', async () => {
    ...
    db.prepare(`
      ...
      VALUES ('route-thread', 'Route thread', 'Route thread subtitle', ?, ?, 1)
    `).run(nowMs, nowMs);
    db.prepare(`
      INSERT INTO feed (
    ...
    assert.strictEqual(data.lastArrangeAtMs, nowMs);
    assert.deepStrictEqual(data.activeThreads?.map((thread) => thread.id), ['route-thread']);
  });

  test('respects limit when later items reuse a live thread id non-contiguously', async () => {
    const db = getDb();
    const nowMs = Date.now();
    db.prepare(`
      INSERT INTO feed_threads (id, title, subtitle, created_at_ms, updated_at_ms, active)
      VALUES
        ('route-repeat-thread', 'Repeat thread', 'Repeat subtitle', ?, ?, 1),
        ('route-other-thread', 'Other thread', 'Other subtitle', ?, ?, 1)
    `).run(nowMs, nowMs, nowMs, nowMs);
    db.prepare(`
      INSERT INTO feed (
        id, type, source, title, text, display_order, thread_id, display_subtitle, published_at, created_at
      )
      VALUES
        ('route-repeat-top', 'article', 'manual', 'Repeat top', 'First repeat item', 1, 'route-repeat-thread', 'Top repeat note', '2026-04-25T13:00:00.000Z', '2026-04-25T13:00:00.000Z'),
        ('route-other-middle', 'article', 'manual', 'Other middle', 'Middle item', 2, 'route-other-thread', 'Middle note', '2026-04-25T12:00:00.000Z', '2026-04-25T12:00:00.000Z'),
        ('route-repeat-later', 'article', 'manual', 'Repeat later', 'Later repeat item', 3, 'route-repeat-thread', 'Later repeat note', '2026-04-25T11:00:00.000Z', '2026-04-25T11:00:00.000Z')
    `).run();

    const response = await GET(new Request('http://127.0.0.1/api/feed?limit=2'));
    assert.strictEqual(response.status, 200);

    const data = await response.json() as FeedListResponse;
    assert.strictEqual(data.limit, 2);
    assert.deepStrictEqual(data.items.map((item) => item.id), ['route-repeat-top', 'route-other-middle']);
