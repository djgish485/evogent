...
import { resolveEvogentBackAction, type EvogentBackLayerState } from '@/lib/evogent-back-navigation';
...
import { getOldestLoadedPrimaryFeedItemTimestamp, mergeFeedItemsForPendingReveal, shouldIncludeConversationTimelineEntry, shouldRenderFeedEmptyState } from '@/lib/feed-normalize';
import { compareThreadGroupItems, countFeedPresentationChanges, countReorganizedItems, hasTruthfulThreadMembers, mergeFeedItemsInServerOrder, pinVisibleFeedItems, shouldStageVisibleFeedReorder, stageFeedArrangementItems } from '@/lib/feed-render-entries';
...
function buildFeedItem(overrides: Partial<FeedItem> & { id: string; type: FeedItemType; createdAt: string }): FeedItem {
  return {
    ...
    threadDisplayEnabled: overrides.threadDisplayEnabled,
    threadId: overrides.threadId,
    displaySubtitle: overrides.displaySubtitle,
    threadTitle: overrides.threadTitle,
    threadSubtitle: overrides.threadSubtitle,
...
const CLOSED_ANDROID_BACK_LAYERS: EvogentBackLayerState = {
  renameSessionModalOpen: false,
  createSessionModalOpen: false,
  codeFixReasoningModalOpen: false,
  brainProviderModalOpen: false,
  usageModalOpen: false,
  mobileMenuOpen: false,
  chatReasoningPopoverOpen: false,
  chatCompactPopoverOpen: false,
  chatSessionMenuOpen: false,
  sessionPickerOpen: false,
  commandPickerOpen: false,
  configOpen: false,
  preferencesOpen: false,
  agentTranscriptExpanded: false,
  detailDepth: 0,
  groupDetailOpen: false,
};

describe('Android Back page-layer routing', () => {
  test('declines Back on the base feed so the native shell can use trusted history/root', () => {
    assert.strictEqual(resolveEvogentBackAction(CLOSED_ANDROID_BACK_LAYERS), null);
  });

  test('maps each visible React surface to one close or pop action', () => {
    const cases: Array<{
      state: Partial<EvogentBackLayerState>;
      action: NonNullable<ReturnType<typeof resolveEvogentBackAction>>;
    }> = [
      { state: { renameSessionModalOpen: true }, action: 'close-rename-session' },
      { state: { createSessionModalOpen: true }, action: 'close-create-session' },
      { state: { codeFixReasoningModalOpen: true }, action: 'close-code-fix-reasoning' },
      { state: { brainProviderModalOpen: true }, action: 'close-brain-provider' },
      { state: { usageModalOpen: true }, action: 'close-usage' },
      { state: { mobileMenuOpen: true }, action: 'close-mobile-menu' },
      { state: { chatReasoningPopoverOpen: true }, action: 'close-chat-reasoning' },
      { state: { chatCompactPopoverOpen: true }, action: 'close-chat-compact' },
      { state: { chatSessionMenuOpen: true }, action: 'close-chat-session-menu' },
      { state: { sessionPickerOpen: true }, action: 'close-session-picker' },
      { state: { commandPickerOpen: true }, action: 'close-command-picker' },
      { state: { configOpen: true }, action: 'close-config' },
      { state: { preferencesOpen: true }, action: 'close-preferences' },
      { state: { agentTranscriptExpanded: true }, action: 'collapse-agent-transcript' },
      { state: { detailDepth: 2 }, action: 'pop-detail' },
      { state: { groupDetailOpen: true }, action: 'close-group-detail' },
    ];

    for (const entry of cases) {
      assert.strictEqual(
        resolveEvogentBackAction({ ...CLOSED_ANDROID_BACK_LAYERS, ...entry.state }),
        entry.action,
      );
    }
  });

  test('closes only the visually top layer and leaves deeper layers for later Back presses', () => {
    assert.strictEqual(
      resolveEvogentBackAction({
        ...CLOSED_ANDROID_BACK_LAYERS,
        brainProviderModalOpen: true,
        mobileMenuOpen: true,
        detailDepth: 2,
...
describe('thread group ordering', () => {
  test('requires two distinct real members before rendering thread chrome', () => {
    const singleton = buildFeedItem({
      id: 'singleton',
      type: 'article',
      createdAt: '2026-06-01T02:03:30.507Z',
    });

    assert.equal(hasTruthfulThreadMembers([singleton]), false);
    assert.equal(hasTruthfulThreadMembers([singleton, singleton]), false);
    assert.equal(hasTruthfulThreadMembers([
      singleton,
      { ...singleton, id: 'second-member' },
    ]), true);
  });

  test('keeps curator display order inside a thread group', () => {
    const speed = buildFeedItem({
      id: 'speed-prototyping',
      type: 'article',
      createdAt: '2026-06-01T02:03:30.507Z',
      displayOrder: 5,
    });
    const cancelling = buildFeedItem({
      id: 'cancelling-ai-subscription',
      type: 'article',
      createdAt: '2026-06-01T02:03:30.506Z',
      displayOrder: 6,
    });

    assert.deepStrictEqual(
      [cancelling, speed].sort(compareThreadGroupItems).map((item) => item.id),
      ['speed-prototyping', 'cancelling-ai-subscription'],
    );
  });

  test('matches backend created-order fallback for unarranged thread groups', () => {
    const older = buildFeedItem({
      id: 'older-thread-item',
      type: 'article',
      createdAt: '2026-06-01T02:03:30.506Z',
    });
    const newer = buildFeedItem({
      id: 'newer-thread-item',
      type: 'article',
      createdAt: '2026-06-01T02:03:30.507Z',
    });

    assert.deepStrictEqual(
      [newer, older].sort(compareThreadGroupItems).map((item) => item.id),
      ['newer-thread-item', 'older-thread-item'],
    );
  });
});

describe('pending feed reveal ordering', () => {
  test('keeps fresh arranged items above unarranged pending items', () => {
    const currentItems = [
      buildFeedItem({ id: 'arranged-lead', type: 'article', createdAt: '2026-06-01T01:00:00.000Z', displayOrder: 1 }),
      buildFeedItem({ id: 'older-unread', type: 'article', createdAt: '2026-05-20T01:00:00.000Z', displayOrder: 2 }),
    ];
    const pendingItems = [
      buildFeedItem({ id: 'fresh-agent-card', type: 'analysis', createdAt: '2026-06-01T02:00:00.000Z' }),
    ];

    const merged = mergeFeedItemsForPendingReveal(currentItems, pendingItems, 'created', {
      lastArrangeAtMs: Date.parse('2026-06-01T02:05:00.000Z'),
      nowMs: Date.parse('2026-06-01T02:06:00.000Z'),
    });
...
describe('feed arrangement refresh helpers', () => {
  test('preserves visible mid-scroll items while staging a new arranged order', () => {
    const currentItems = [
      buildFeedItem({ id: 'old-lead', type: 'article', createdAt: '2026-06-01T01:00:00.000Z', displayOrder: 1 }),
      buildFeedItem({ id: 'visible-a', type: 'article', createdAt: '2026-06-01T01:01:00.000Z', displayOrder: 2 }),
      buildFeedItem({ id: 'visible-b', type: 'article', createdAt: '2026-06-01T01:02:00.000Z', displayOrder: 3 }),
      buildFeedItem({ id: 'old-tail', type: 'article', createdAt: '2026-06-01T01:03:00.000Z', displayOrder: 4 }),
    ];
    const serverItems = [
      buildFeedItem({ id: 'new-lead', type: 'article', createdAt: '2026-06-01T02:00:00.000Z', displayOrder: 1 }),
      buildFeedItem({ id: 'visible-b', type: 'article', createdAt: '2026-06-01T01:02:00.000Z', displayOrder: 2 }),
      buildFeedItem({ id: 'new-support', type: 'article', createdAt: '2026-06-01T02:01:00.000Z', displayOrder: 3 }),
      buildFeedItem({ id: 'visible-a', type: 'article', createdAt: '2026-06-01T01:01:00.000Z', displayOrder: 4 }),
      buildFeedItem({ id: 'old-tail', type: 'article', createdAt: '2026-06-01T01:03:00.000Z', displayOrder: 5 }),
    ];

    const serverOrderedItems = mergeFeedItemsInServerOrder(serverItems, currentItems);
    const pinnedItems = pinVisibleFeedItems(serverOrderedItems, currentItems, new Set(['visible-a', 'visible-b']));

    // old-lead was INSIDE the snapshot window and is absent from the server list = it was
    // REMOVED server-side (dismissed/disliked/expired). It must NOT be re-appended — retaining
    // it kept dismissed cards on the glass indefinitely.
    assert.deepStrictEqual(pinnedItems.map((item) => item.id), [
      'new-lead',
      'visible-a',
      'visible-b',
      'new-support',
    ...
    assert.strictEqual(countReorganizedItems(serverOrderedItems, currentItems), 3);
  });

  test('stages every visible reorder unless the user explicitly requested it', () => {
    const currentItems = [
      buildFeedItem({ id: 'old-lead', type: 'article', createdAt: '2026-06-01T01:00:00.000Z', displayOrder: 1 }),
      buildFeedItem({ id: 'older-unread', type: 'article', createdAt: '2026-06-01T00:30:00.000Z', displayOrder: 2 }),
    ];
    const serverItems = [
      buildFeedItem({ id: 'new-lead', type: 'article', createdAt: '2026-06-01T02:00:00.000Z', displayOrder: 1 }),
      buildFeedItem({ id: 'older-unread', type: 'article', createdAt: '2026-06-01T00:30:00.000Z', displayOrder: 2 }),
      buildFeedItem({ id: 'old-lead', type: 'article', createdAt: '2026-06-01T01:00:00.000Z', displayOrder: 3 }),
    ];

    const serverOrderedItems = mergeFeedItemsInServerOrder(serverItems, currentItems);

    assert.deepStrictEqual(serverOrderedItems.map((item) => item.id), ['new-lead', 'older-unread', 'old-lead']);
    assert.strictEqual(countReorganizedItems(serverOrderedItems, currentItems), 1);
    assert.strictEqual(shouldStageVisibleFeedReorder(2), true);
    assert.strictEqual(shouldStageVisibleFeedReorder(2, true), false);
    assert.strictEqual(shouldStageVisibleFeedReorder(0), false);
    assert.strictEqual(
      countReorganizedItems(serverOrderedItems, currentItems, new Set(['old-lead', 'older-unread'])),
      0,
      'the notice must count the unpinned server diff, not this filtered value',
    );

    const visibleSwap = [
      { ...currentItems[1]!, displayOrder: 1 },
      { ...currentItems[0]!, displayOrder: 2 },
    ];
    assert.strictEqual(
      countReorganizedItems(visibleSwap, currentItems),
      2,
      'a visible-only swap still needs a Show new order notice',
    );
  });

  test('staging freezes render-order and thread fields while reconciling updates and removals', () => {
    const currentItems = [
      buildFeedItem({
        id: 'visible-thread-a',
        type: 'article',
      ...
      buildFeedItem({ id: 'removed', type: 'article', displayOrder: 3 }),
    ];
    const serverItems = [
      buildFeedItem({ id: 'new-lead', type: 'article', displayOrder: 1 }),
      buildFeedItem({
        id: 'visible-thread-b',
        type: 'article',
        displayOrder: 2,
        threadDisplayEnabled: false,
        threadId: null,
        threadTitle: null,
      }),
      buildFeedItem({
    ...
    const staged = stageFeedArrangementItems(serverItems, currentItems);
    assert.deepStrictEqual(staged.map((item) => item.id), [
      'visible-thread-a',
      'visible-thread-b',
    ]);
    assert.strictEqual(staged[0]?.text, 'fresh copy', 'non-presentation updates should reconcile');
    assert.strictEqual(staged[0]?.displayOrder, 1);
    assert.strictEqual(staged[0]?.threadDisplayEnabled, true);
    assert.strictEqual(staged[0]?.threadId, 'thread-old');
    assert.strictEqual(staged[0]?.threadTitle, 'Old thread');
    assert.strictEqual(countFeedPresentationChanges(serverItems, currentItems), 3);
  });
});
...
describe('feed conversation timeline composition', () => {
  ...
  test('does not surface conversation cards before the initial All feed load settles', () => {
    assert.strictEqual(
      shouldIncludeConversationTimelineEntry({
        selectedFilter: 'all',
        oldestLoadedPrimaryFeedItemTimestamp: null,
        conversationLastTimestamp: '2026-04-28T10:00:00.000Z',
        isInitialPrimaryFeedLoading: true,
      }),
      false,
    );
    assert.strictEqual(
      shouldRenderFeedEmptyState({ isLoading: true, visibleFeedEntryCount: 0 }),
      false,
    );
  });
  ...
  test('keeps mixed All pagination bounded once primary feed items are loaded', () => {
    ...
    // Anchor "now" to the fixture dates: idle-session staleness is judged
    // against real time, not against the loaded-content boundary.
