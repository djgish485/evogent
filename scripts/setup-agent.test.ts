...
test('setup agent report prints hard required provider and source lines', () => {
  const readiness: FirstRunReadiness = {
    ...
    pending: ['Run node scripts/create-default-sessions.mjs after Evogent is running.'],
    ready: [],
  };
  ...
  assert.match(report, /^PENDING default_sessions: Run node scripts\/create-default-sessions\.mjs after Evogent is running\./m);
  ...
  assert.match(codingAgentOnlyReport, /^PENDING default_sessions: Run node scripts\/create-default-sessions\.mjs --coding-agent-only after Evogent is running\./m);
});
...
test('setup agent report names bootstrapped default chat sessions', () => {
  const readiness: FirstRunReadiness = {
    ...
    sessions: {
      ready: true,
      mainSessionId: 'general-session-id',
      curatorSessionId: 'curator-session-id',
    },
    sources: {
      ready: true,
      items: [{
  ...
  assert.match(report, /^READY default_sessions: General Agent=general-session-id Curator Agent=curator-session-id$/m);
});
