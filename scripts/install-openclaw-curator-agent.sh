...
write_curator_agents_file() {
  ...
  if [[ -n "$playbook_file" && ! -f "$playbook_file" ]]; then
    echo "Required Evogent playbook file missing: $playbook_file" >&2
    exit 1
  fi

  AGENTS_SOURCE_FILE="$source_file" \
  AGENTS_TARGET_FILE="$target_file" \
  AGENTS_PLAYBOOK_FILE="${playbook_file:-}" \
  node <<'NODE'
...
const playbookFile = process.env.AGENTS_PLAYBOOK_FILE;
const sectionHeading = '## Beyond installed skills — exploration';
const playbookHeading = '## Phased curation playbook (formerly /curate command)';

const explorationSection = [
  sectionHeading,
  '',
  '- MANDATORY first step every cycle (before any submission decisions): run bash `curl -s "$MEDIA_AGENT_INTERNAL_BASE_URL/api/feed?agentEvidence=1&limit=200&sort=created" | jq -r \'.items[] | "\\(.createdAt[:16]) | \\(.source) | \\(.title)"\'` and `sqlite3 /root/media-agent/data/media-agent.db "SELECT source_id, url FROM feed WHERE NOT (type = \'notification\' AND source = \'phone-notification\') AND created_at_ms > (strftime(\'%s\',\'now\',\'-72 hours\')*1000) AND (source_id IS NOT NULL OR url IS NOT NULL) LIMIT 200"`. By policy, phone-notification title/body/app content must not be curator evidence. The current worker shares the server\'s Unix UID and storage, so API/SQL filtering is not an OS sandbox; never bypass the exclusion with shell, filesystem, or broader SQLite reads. Hold the resulting titles + source_ids + urls in mind. Do not resubmit any card whose underlying real-world fact already appears in either list, regardless of title wording.',
  '- MANDATORY carry-forward read in the same first-step block: call `evogent_feed_carry_forward({ "includeAllUnviewed": true, "includeDisplayed": true })` before selecting final cards. Complete-set mode returns every structurally eligible accepted-but-unviewed primary row regardless of age, including rows already displayed but still unread. Its order preserves prior explicit shipment order and then stable evidence sequence; it is not a mechanical value ranking. If the tool is unavailable, use `/root/media-agent/data/media-agent.db` as the fallback DB path and query the full unviewed primary feed set yourself. Do not query `data/evogent.db`; that is not the Evogent feed database.',
  '',
  ...
  '7. Use `metadata.kind: "observation"` ONLY for cards that bridge at least two distinct sources where one is the user\'s private data (Gmail, Calendar, chat history, or interactions). Pure news analysis without a private-data bridge is `metadata.kind: "analysis"`. Use `metadata.bridges` as an array of source-kind strings (e.g. `["gmail","twitter"]`), not as a tagline string.',
  '',
  '8. Read-only on private data. Do not call `gog gmail send/draft/trash/modify`, any Gmail label/batch mutation, or any calendar create/delete/mutation.',
  '',
  '9. `originSessionId` links a feed item to the chat session that produced it. For scheduled or autonomous curator items, omit `originSessionId` entirely. If the item was produced inside an OpenClaw curator chat, use that real bridge session id, such as `openclaw:agent:curator:main` or `openclaw:agent:curator:cron:<cron-id>`. Do not invent run tags like `curator-webchat-2026-05-18T02:00Z`, `cron:<id>`, or `gateway-client-<timestamp>`.',
  '',
  '10. Stable identity for cross-source observation cards: build `sourceId` from the underlying real-world FACT, not from the title (e.g. `evogent-obs:calendar:<eventId>`, `evogent-obs:gmail-renewal:<vendor>:<date>`). The same real-world fact across cycles must produce the same sourceId so the existing dedup path collapses it to one card.',
  '',
  '- QUALITY, NOT VOLUME: there is no minimum item count, source/type mix, or thread count. The arranged primary slate has a hard ceiling of 50 items, not a target. A zero-item, one-item, or small cycle is correct when that is the agent\'s honest judgment after the required reads. Never pad, manufacture threads, or lower the quality bar to hit a number.',
  '- OUTCOME RECEIPT: every automated cycle must end with one terminal `evogent_feed_submit` call carrying a complete `cycleSummary`, even when `items` is empty. Include truthful `considered`, `selected`, `rejected`, and `topRejectionReasons`; include rejected candidate receipts when candidates were actually judged. For an empty cycle, `topRejectionReasons` must concretely say why nothing qualified. Earlier chunk submits, if any, must omit `cycleSummary` so only the terminal receipt closes the cycle.',
  '- Mine the cache as deeply as the current evidence and judgment require. Do not stop merely because the first angle is sparse, and do not keep querying merely to fill the slate.',
  '- Do not use your final turn for a long wrap-up response. The feed tools are the output surface; submit and arrange the items instead.',
  '',
  '- MANDATORY at the END of every cycle (after all submissions): call `evogent_feed_arrange` with your ordering + thread decisions, even when nothing new was submitted. Silent skip = the user sees a frozen feed.',
  '- MANDATORY when calling `evogent_feed_arrange`: `ordering[]` MUST let newly submitted items and the strongest older unread IDs from the all-unviewed carry-forward review compete on judged value. Do not limit this judgement to the latest feed page; the carry-forward review is the full eligible set. Include every older unread item that still belongs near the top, with no fixed minimum, and omit weaker ones with an honest reason. If the user has viewed an item already, it should DROP in the ordering even if it was newly submitted.',
  '- Before `evogent_feed_arrange`, compare the final `ordering[]` against the `evogent_feed_carry_forward` ids. Include the still-important carry-forward items where their judged value places them, or explicitly drop them when they are now stale/low-value. The arrange endpoint has a safety backstop, but relying on the backstop means you failed to make the editorial judgement yourself.',
  '- Include carry-forward judgement in your feed tool response payload under `cycleSummary.carryForwardAudit` when possible: `{ includeAllUnviewed, includeDisplayed, eligibleCount, reviewedCount, returnedCount, reviewLimit, promotedIds, droppedImportantIds, dropReasons }`. This preserves the why: "older by publish date" is different from "accepted earlier and carried forward because the user has not viewed it."',
  ...
  '- For threads, group only when grouping HELPS the user and the items share one real editorial angle. Leave unrelated or standalone items as singletons. Give a continuing angle a stable thread id, keep every thread as one unsplit shipment, and never create, split, or pad a thread to hit a count.',
  ...
  '- Threads are live current-cycle groupings. Deactivate threads that no longer have relevant material. Reuse a stable thread id when the same editorial angle continues; change membership only when the current judgment genuinely changes.',
...
function stripPlaybookPreamble(content) {
  // The /curate command file starts with execution-model / Usage preamble that
  // is irrelevant inside an OpenClaw curator session. Start the bundled playbook
  // at the first numbered section ("## 0. ...") so the agent only sees the phases.
  const startIndex = content.search(/^## 0\.\s/m);
  if (startIndex === -1) {
    return content.trimEnd();
  }
...
let playbookContent = '';
if (playbookFile) {
  const raw = fs.readFileSync(playbookFile, 'utf8');
  playbookContent = `${playbookHeading}\n\n${stripPlaybookPreamble(raw)}`;
}
fs.mkdirSync(path.dirname(targetFile), { recursive: true });
const parts = [baseContent, playbookContent, explorationSection].filter(Boolean);
...
write_curator_agents_file "$repo_dir/data/curation-prompt.md" "$agent_dir/AGENTS.md" "$repo_dir/.claude/commands/curate.md"
