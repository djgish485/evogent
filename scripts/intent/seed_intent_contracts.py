...
CONTRACTS = [
    ...
    ("feed_ordering",
     ...
     "Dan 2026-06-11 verdicts on the May-3 Codex tweet ('totally out of date') and Apr-22 Tim Cook tweet ('total nonsense') vs 'older interesting tweets' being welcome. Rubric: data/interestingness-rubric.md.",
    ...
    ("feed_ordering",
     ...
     "established-by-user", "fixed-2026-06-11"),
    ("meta",
     "Every curated item carries metadata.interest. History was bootstrap-scored against data/interestingness-rubric.md (scoredBy claude-bootstrap-20260611, 9,666 items); curator-stamped scores always win over bootstrap; unscored fresh items default to 0.55 and are counted in carry_forward_audit.unscoredFreshCount as a compliance signal.",
    ...
    ("read_viewed",
     ...
     "established-by-user", "open"),
    ("feed_ordering",
     "Own @djgish tweets/replies are taste signals to mine, never feed items to surface. Non-English content is filtered or translated, never silently included. Same-topic thread repetition across consecutive cycles is a miss even in a loved lane.",
     "Dan 2026-04-19: 'Why were these in the feed? They are not interesting to me on their own, I made them!'; 2026-04-14 German YouTube; 2026-04-19: 'the curating agent keeps creating these Iran threads. It's getting boring.' (All from in-app chats, ingested 2026-06-11.)",
     "No author_username like %djgish% in arranged slate; rubric v1.1 hard-punish list covers all three.",
     "established-by-user", "fixed-2026-06-11"),
    ("curation_volume",
     "Controlled variance is intended, not accidental: most curations should include one deliberately borderline/high-variance thread, visibly marked for easy A/B rating, so feedback can evolve future curations. (Founding-era intent, Apr 2026; never implemented.)",
     "Dan 2026-04-26: 'maybe you should be encouraged to have a thread in most curations that you're not sure whether to include... bigger obvious A/B buttons where the user lets you know'; 2026-04-27: 'encourage like a random/etc thread in curations sometimes'.",
     "Curation prompt gains a borderline-thread instruction; UI shows rate-me affordance on it.",
     "established-by-user", "open"),
    ("meta",
     "LAW (permanently in force, never 'completed'): agent instructions are preferred over brittle deterministic code wherever possible; deterministic code is reserved for floors, bookkeeping, and contracts that agents have proven unable to keep.",
     "Dan 2026-04-14: 'is against the development philosophy! I want everything in the caching agent instructions if possible'; 2026-05-04 dev philosophy statement; reaffirmed 2026-06-10: 'I don't want a bunch of deterministic checks tho.'",
     "Code review question: could this new deterministic logic be agent instructions instead? Burden of proof is on the code.",
     "established-by-user", "law"),
    ("meta",
     "LAW: agents verify by running the real thing before claiming or suggesting. Manual emulation/browser checks beat passing tests; 'no failing tests' is never proof of intent alignment.",
     "Dan 2026-04-14/15/18 repeated: agents 'must manually verify their assumptions by running things before submitting code fix suggestions'; 2026-04-19: emulate the curation with the same instructions the automated system uses.",
     "Every fix ships with evidence of live verification (DB state, browser, real run).",
     "established-by-user", "law"),
    ("meta",
     "LAW: the repo is self-contained — all orchestration, scripts, and config templates live inside the repo for future-user portability; nothing load-bearing hides in VM home directories.",
     "Dan 2026-04-14: pivoted from tolerating ~/.clawdbot scripts to insisting all orchestration files live in the repo for future users.",
     "New scripts/units/config land under the repo tree; install scripts derive paths from install location.",
     "established-by-user", "law"),
    ("meta",
     "LAW: explain simply, no invented jargon. Titles, threads, analysis, and docs use plain language with named actors; obfuscating compound nouns are the tell of non-understanding.",
     "Dan 2026-04-20: 'WTF kind of title is this... Codex especially tends towards jargon and obfuscating language'; AGENTS.md Explain Simply section.",
     "Spot-check shipped titles/rationales for jargon; curate.md word-count and plain-language gates.",
     "established-by-user", "law"),
    ("meta",
     "The repo is the product and must generalize to ANY user; Dan is the test user. Personal taste, config, and data live in the user data layer (data/*.md instances, app DB, intent ledger) - never committed. Committed files ship mechanisms, templates (*.default.md), and build processes, not Dan's instances.",
     "Dan 2026-06-11: 'the app is built for public consumption... everything needs to be generalizable to ANYONE using the app.' Precedent: curation-prompt.default.md (committed) vs curation-prompt.md (user data). Violation fixed same day: personal rubric moved out of docs/.",
     "git ls-files contains no personal-taste instances; *.default.md templates exist for every per-user file the system reads.",
     "established-by-user", "fixed-2026-06-11"),
    ("meta",
     "The interestingness rubric is a living document: reflection cycles and new in-app feedback update data/interestingness-rubric.md (versioned), and the curator + backstop read it at runtime, so taste evolves without code changes.",
     "Founding development philosophy (Dan 2026-05-04): agent instructions over brittle code; 'learns what you are trying to understand'. Rubric v1.1 added the evolution section 2026-06-11.",
     "Rubric version bumps appear in git history; reflection notes reference rubric updates.",
     "established-by-user", "open"),
    # --- Meta / process ---
    ("meta",
