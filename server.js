...
} = require('./src/lib/config-apply-tasks.js');
const { failStaleQueuedChatMessages } = require('./lib/chat-message-cleanup.js');
const { reconcileOrphanedPhoneCurations } = require('./lib/phone-startup-reconciliation.js');
const {
  PhoneAuthError,
...
const {
  ...
  getRuntimeProfile,
  isPhoneRuntime,
} = require('./src/lib/runtime-profile');
const {
...
// Default to loopback. The Android shell uses the numeric loopback address so it does not need
// a dual-stack wildcard merely to work around localhost resolver ordering.
const listenHost = getListenHost();
const port = Number.parseInt(process.env.PORT || '3001', 10);
const phoneRuntime = isPhoneRuntime();
const phoneHttpsPort = Number.parseInt(process.env.EVOGENT_PHONE_HTTPS_PORT || '3443', 10);
// Server-side route modules make loopback HTTP calls back into this process. On the phone,
// loopback is shared by every Android app, so source address is not identity. Give only this
// process a fresh, restart-bound credential before Next loads any server route module.
const serverLoopbackSecret = phoneRuntime ? randomBytes(32).toString('base64url') : '';
if (phoneRuntime) {
  process.env.EVOGENT_SERVER_LOOPBACK_SECRET = serverLoopbackSecret;
}

const app = next({ dev, hostname, port });
const handle = app.getRequestHandler();
const internalBaseUrl = phoneRuntime
  ? `http://127.0.0.1:${port}`
...
const adaptiveHeartbeatMode = getAdaptiveHeartbeatMode();
...
const phoneLoopbackAuth = phoneRuntime
  ? createPhoneLoopbackAuth({ tokenFile: dataPath('control-token.txt') })
  : null;
const serverLoopbackSecretPath = dataPath('server-loopback-secret');
...
function readPhoneTlsOptions() {
  if (!phoneRuntime) return null;

  if (
    !Number.isInteger(phoneHttpsPort)
    || phoneHttpsPort < 1
    || phoneHttpsPort > 65535
    || phoneHttpsPort === port
  ) {
    throw new Error('EVOGENT_PHONE_HTTPS_PORT must be a distinct valid TCP port');
  }

  const certPathValue = normalizeOptionalEnv(process.env.EVOGENT_PHONE_TLS_CERT_PATH);
  const keyPathValue = normalizeOptionalEnv(process.env.EVOGENT_PHONE_TLS_KEY_PATH);
  if (!certPathValue || !keyPathValue) {
    throw new Error(
      'Phone runtime requires EVOGENT_PHONE_TLS_CERT_PATH and EVOGENT_PHONE_TLS_KEY_PATH',
    );
  }

  const certPath = path.resolve(certPathValue);
  const keyPath = path.resolve(keyPathValue);
  const certStat = fs.lstatSync(certPath);
  const keyStat = fs.lstatSync(keyPath);
  if (
    certStat.isSymbolicLink()
    || keyStat.isSymbolicLink()
    || !certStat.isFile()
    || !keyStat.isFile()
  ) {
    throw new Error('Phone TLS certificate and key must be regular files');
  }
  if ((keyStat.mode & 0o077) !== 0) {
    throw new Error('Phone TLS private key permissions must not grant group or other access');
  }
  ...
  if (!parsedCert.checkPrivateKey(privateKey)) {
    throw new Error('Phone TLS certificate and private key do not match');
  }
  if (parsedCert.checkIP('127.0.0.1') !== '127.0.0.1') {
    throw new Error('Phone TLS certificate is not valid for exact loopback IP 127.0.0.1');
  }
  ...
  if (
    ...
    throw new Error('Phone TLS certificate is outside its validity window');
  }
...
function publishServerLoopbackSecret() {
  if (!phoneRuntime || !serverLoopbackSecret) return;
  fs.mkdirSync(dataDir, { recursive: true, mode: 0o700 });
  const temporaryPath = `${serverLoopbackSecretPath}.${process.pid}.${randomUUID()}.tmp`;
  fs.writeFileSync(temporaryPath, `${serverLoopbackSecret}\n`, {
    encoding: 'utf8',
    mode: 0o600,
    flag: 'wx',
  });
  fs.chmodSync(temporaryPath, 0o600);
  fs.renameSync(temporaryPath, serverLoopbackSecretPath);
  fs.chmodSync(serverLoopbackSecretPath, 0o600);
}
...
function reconcilePhoneCodeFixQueueOnStartup() {
  if (!isPhoneRuntime()) return;
  const db = getChatStatusDb();
  const schemaReady = db.prepare(`
    SELECT 1
    FROM sqlite_master
    WHERE type = 'table' AND name = 'code_fix_tasks'
  `).get();
  // A truly fresh data directory is initialized by the first Next route request after the
  // listener comes up. There is nothing to reconcile before that schema exists.
  if (!schemaReady) return;
  const activeRows = db.prepare(`
    SELECT suggestion_id AS suggestionId, task_id AS taskId
    FROM code_fix_tasks
  ...
  const reconcile = db.transaction(() => {
    const updateTask = db.prepare(`
      ...
      SET status = 'queued',
          phase = 'host_review',
          phase_detail = 'Queued for the host development workflow',
          error = NULL,
          completed_at = NULL
      WHERE task_id = ?
    `);
    const updateFeed = db.prepare(`
      ...
      SET metadata = json_set(
        ...
        '$.codeFixOrchestratorStatus', 'host_review',
        '$.developmentQueueState', 'queued_for_host',
        '$.taskId', ?
      )
      WHERE id = ?
    `);
    for (const row of activeRows) {
      updateTask.run(row.taskId);
      updateFeed.run(row.taskId, row.suggestionId);
    }
  });
  reconcile();
  console.log(`[code-fix] reconciled ${activeRows.length} on-phone task(s) into the host development queue`);
}
...
function reconcilePhoneCurationLogOnStartup() {
  if (!isPhoneRuntime()) return;

  try {
    const changes = reconcileOrphanedPhoneCurations(getChatStatusDb());
    if (changes > 0) {
      console.warn(
        `[curation-cleanup] Marked ${changes} ownerless phone curation${changes === 1 ? '' : 's'} as aborted on startup`,
      );
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`[curation-cleanup] Failed to reconcile ownerless phone curations: ${message}`);
  }
}

function reconcileConfigApplyTasksOnStartup() {
  try {
    const recovery = reconcileRunningConfigApplyTasks(getChatStatusDb(), { dataDir });
    for (const action of recovery.actions) {
      console.warn(`[config-apply-recovery] task=${action.taskId} action=${action.action}`);
    }
    return recovery.queuedTaskIds;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`[config-apply-recovery] startup reconciliation failed: ${message}`);
    return [];
  }
}

async function resumeConfigApplyTasksOnStartup(taskIds) {
  for (const taskId of Array.isArray(taskIds) ? taskIds : []) {
    try {
      if (backgroundJobsDisabled) {
        await postInternal('/api/internal/config-apply-tasks/execute', { taskId });
      } else {
        await enqueueBackgroundJob(BACKGROUND_JOB_NAMES.CONFIG_APPLY, { taskId }, {
          jobId: `config-apply-${taskId}`,
        });
      }
      console.log(`[config-apply-recovery] resumed task=${taskId}`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
...
function isLocalRequest(req) {
  // Loopback is a transport location, not an identity boundary on shared-device hosts.
  const raw = (req.socket.remoteAddress || '').toLowerCase();
  const addr = raw.replace(/^::ffff:/, '');
  return addr === '::1' || addr === '0:0:0:0:0:0:0:1' || addr.startsWith('127.');
}
...
function isServerInternalRequest(req) {
  if (!phoneRuntime || !isLocalRequest(req)) return false;
  return safeEqualString(
    getHeaderString(req, 'x-evogent-server-secret'),
    serverLoopbackSecret
  );
}
...
// Every install gets one durable "main" session — the standing target for the pinned
// composer. Best-effort at boot (tables may not exist on a brand-new install yet);
// the chat route's getOrCreateMainChatSession fallback is the lazy guarantee.
function ensureMainChatSessionSeed() {
  try {
    const db = getChatStatusDb();
    const hasSessions = db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='chat_sessions'").get();
    if (!hasSessions) return;
    const existing = db.prepare("SELECT id FROM chat_sessions WHERE session_type = 'main' LIMIT 1").get();
    if (existing) return;

    let provider = 'claude';
    try {
      const { readBrainConfig } = require('./lib/brain-config.js');
      provider = readBrainConfig(dataPath('config.md')).provider === 'codex' ? 'codex' : 'claude';
    } catch { /* default to claude */ }

    const id = randomUUID();
    db.prepare(`
      INSERT INTO chat_sessions (id, provider, provider_session_id, claude_session_id, title, session_type)
      VALUES (?, ?, ?, ?, 'Evogent', 'main')
    `).run(id, provider, id, provider === 'claude' ? id : '');
    const hasSettings = db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='chat_session_brain_settings'").get();
    if (hasSettings) {
      db.prepare(`
        INSERT OR IGNORE INTO chat_session_brain_settings (session_id, claude_reasoning_effort, codex_reasoning_effort, codex_fast_mode)
        VALUES (?, 'low', 'low', 0)
      `).run(id);
    }
    console.log('> Seeded main chat session (Evogent)');
  } catch (err) {
    console.warn('[main-session-seed] skipped:', err?.message || err);
  }
}
...
function completeCurationLogForTask(event) {
  return failPendingAutomatedCurationWithoutReceipt(getChatStatusDb(), event);
}
...
// new rollout and persists the real server-assigned id from thread.started. Persisting a
// client-minted id would make the following turn resume a non-existent rollout.
function clearChatSessionProviderSessionId(sessionId, providerName) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`[orchestrator] Failed to clear chat session ID (${sessionId}): ${message}`);
    return false;
  }
}
...
function readBoundedJsonBody(req, maxBytes = 8192) {
  return new Promise((resolve, reject) => {
    const declaredLength = Number.parseInt(getHeaderString(req, 'content-length') || '0', 10);
    if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
      reject(new PhoneAuthError('body_too_large', 'Authentication request is too large'));
      req.resume();
      return;
    }
    ...
    const timeout = setTimeout(() => {
      if (settled) return;
      settled = true;
      reject(new PhoneAuthError('request_timeout', 'Authentication request timed out'));
      req.destroy();
    }, 5000);
    ...
    req.on('data', (chunk) => {
      ...
      size += chunk.length;
      if (size > maxBytes) {
        finish(() => reject(new PhoneAuthError('body_too_large', 'Authentication request is too large')));
        req.resume();
        return;
      }
      chunks.push(chunk);
    ...
    req.on('end', () => {
      finish(() => {
        try {
          ...
          reject(new PhoneAuthError('invalid_request', 'Invalid authentication request'));
        }
      });
    });
    req.on('error', (error) => {
    ...
    req.on('aborted', () => {
      finish(() => reject(new PhoneAuthError('invalid_request', 'Authentication request aborted')));
    });
  });
}

function sendPhoneAuthJson(res, statusCode, payload, extraHeaders = {}) {
...
function getPhoneAuthErrorStatus(error) {
  if (!(error instanceof PhoneAuthError)) return 400;
  if (error.code === 'rate_limited') return 429;
  if (error.code === 'unavailable') return 503;
  if (error.code === 'body_too_large') return 413;
  if (error.code === 'invalid_transport') return 403;
  if (error.code === 'invalid_challenge' || error.code === 'invalid_proof') return 401;
  return 400;
}

async function handlePhoneAuthRequest(req, res, pathname) {
  if (!phoneRuntime || !phoneLoopbackAuth) return false;
  if (
    pathname !== '/api/phone-auth/challenge'
    && pathname !== '/api/phone-auth/complete'
  ) {
    return false;
  }

  if (!isLocalRequest(req)) {
    sendPhoneAuthJson(res, 403, { ok: false, error: 'Forbidden' });
    return true;
  }
  if (req.method !== 'POST') {
    sendPhoneAuthJson(
  ...
  if (!contentType.startsWith('application/json')) {
    sendPhoneAuthJson(res, 415, { ok: false, error: 'JSON required' });
    return true;
  }

  try {
    const body = await readBoundedJsonBody(req);
    const remoteKey = (req.socket.remoteAddress || 'loopback').toLowerCase();
    if (pathname.endsWith('/challenge')) {
      const challenge = phoneLoopbackAuth.issueChallenge(body.clientNonce, remoteKey);
      sendPhoneAuthJson(res, 200, challenge);
      return true;
    }

    if (body?.sessionKind === 'web' && req.socket.encrypted !== true) {
      throw new PhoneAuthError(
        'invalid_transport',
        'Web sessions require the release-pinned HTTPS listener',
      );
    }
    const session = phoneLoopbackAuth.completeChallenge(body, remoteKey);
    const headers = session.sessionKind === 'web'
      ? { 'Set-Cookie': phoneLoopbackAuth.getWebCookie(session) }
      : {};
    sendPhoneAuthJson(res, 200, session, headers);
  } catch (error) {
    const status = getPhoneAuthErrorStatus(error);
    // Authentication material and transcript fields are deliberately never logged.
    sendPhoneAuthJson(res, status, {
      ok: false,
      error: status === 429
...
async function runAdaptiveHeartbeatCheck(triggeredBy = 'timer') {
  ...
  try {
    ...
    if (result?.triggered) {
      console.log(`[adaptive-heartbeat] triggered curator run: ${result.triggerReason || 'triggered'}`);
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
function startAdaptiveHeartbeatTimer() {
  ...
  console.log('> Adaptive curation heartbeat scheduled (15min interval)');
}

const ARRANGE_BACKSTOP_INTERVAL_MS = 5 * 60 * 1000;
const ARRANGE_BACKSTOP_LOOKBACK_MS = 30 * 60 * 1000;
let arrangeBackstopInFlight = false;

async function runArrangeBackstop() {
  if (arrangeBackstopInFlight) return;
  arrangeBackstopInFlight = true;
  try {
    const startedAtMs = Date.now() - ARRANGE_BACKSTOP_LOOKBACK_MS;
    const result = await postInternal('/api/internal/feed/arrange-backstop', { startedAtMs });
    if (result?.fired === true) {
      console.log(`[arrange-backstop:timer] fired itemCount=${result.itemCount} reason=${result.reason}`);
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.warn(`[arrange-backstop:timer] check failed: ${message}`);
  } finally {
    arrangeBackstopInFlight = false;
  }
}

function startArrangeBackstopTimer() {
  const startupTimer = setTimeout(() => {
    void runArrangeBackstop();
  }, 60_000);
  if (typeof startupTimer.unref === 'function') startupTimer.unref();

  const timer = setInterval(() => {
    void runArrangeBackstop();
  }, ARRANGE_BACKSTOP_INTERVAL_MS);
  if (typeof timer.unref === 'function') timer.unref();

  console.log('> Arrange backstop scheduled (5min interval, 30min lookback)');
}
...
app.prepare().then(async () => {
  const requestHandler = async (req, res) => {
    ...
    if (await handlePhoneAuthRequest(req, res, parsedUrl.pathname)) {
      return;
    }

    // Android loopback is a shared network namespace, not a caller identity. In the phone
    // runtime every document, asset, API, SSE, and write must carry either a process-bound
    // phone session or the server's own restart-bound internal credential.
    if (
      phoneRuntime
      && !isServerInternalRequest(req)
      && !phoneLoopbackAuth.authorizeRequest(req, {
        allowDirect: true,
        allowWeb: req.socket.encrypted === true,
      })
    ) {
      sendPhoneAuthJson(res, 401, {
        ok: false,
        error: 'Phone session required',
      }, {
    ...
    if (!phoneRuntime && !isPublicReadPath(req.method, parsedUrl.pathname) && !trustNetwork && !(await isTrustedSocket(req))) {
      // A denial here surfaces to the user as a red "sign in" error on every write action —
      // log WHO was denied so a loopback-detection miss is diagnosable instead of invisible.
      console.warn(`[trust-gate] denied ${req.method} ${parsedUrl.pathname} remoteAddress=${req.socket.remoteAddress} cf=${Boolean(req.headers['cf-connecting-ip'] || req.headers['cf-ray'])}`);
      res.statusCode = 403;
    ...
    if (req.method === 'POST' && parsedUrl.pathname === '/api/internal/cache-refresh/pre-curation') {
      ...
      try {
        ...
        if (Number.isInteger(body?.timeoutMs) && body.timeoutMs > 0) {
          options.timeoutMs = Math.min(body.timeoutMs, 4 * 60 * 1000);
        }
    ...
    if (req.method === 'POST' && parsedUrl.pathname === '/api/internal/code-fix-orchestrator/enqueue') {
      ...
      try {
        ...
        const hostQueueMode = phoneRuntime;
        ...
        for (const suggestion of valid) {
          ...
          if (activeTaskRow && typeof activeTaskRow.taskId === 'string' && activeTaskRow.taskId.trim()) {
            ...
            responseStatuses.push(hostQueueMode && activeTaskRow.status === 'queued' ? 'accepted' : activeTaskRow.status);
            continue;
          }
          ...
          taskIds.push(taskId);
          responseStatuses.push(hostQueueMode ? 'accepted' : 'dispatched');
          ...
          try {
            db.prepare(
              ...
              hostQueueMode ? 'accepted' : 'dispatched',
              hostQueueMode ? 'host_review' : 'dispatched',
              taskId,
              suggestion.id,
            );
            if (hostQueueMode) {
          ...
          upsertCodeFixTaskRow(db, {
            ...
            taskId,
            status: hostQueueMode ? 'queued' : 'dispatched',
            phase: hostQueueMode ? 'host_review' : 'queued',
            phaseDetail: hostQueueMode
              ? 'Queued for the host development workflow'
              : 'Queued for direct dev-task dispatch',
            error: null,
            completedAt: null,
          });
          ...
          if (hostQueueMode) {
            broadcastAgentProgress({
              event: {
                event: 'code_fix_host_queue_updated',
                taskId,
        ...
        const responseSuggestionStatus = responseStatuses.includes('accepted')
          ? 'accepted'
          : responseStatuses.includes('dispatched')
          ? 'dispatched'
    ...
    if (req.method === 'GET' && parsedUrl.pathname === '/api/internal/code-fix-orchestrator/host-queue') {
      res.setHeader('Content-Type', 'application/json');
      try {
        const db = getChatStatusDb();
        const tasks = db.prepare(`
          SELECT
            t.task_id AS taskId,
            t.suggestion_id AS suggestionId,
            t.started_at AS queuedAt,
            t.phase_detail AS phaseDetail,
            f.title,
            f.text,
            f.metadata,
            f.origin_session_id AS originSessionId
          ...
          WHERE t.status = 'queued' AND t.phase = 'host_review'
          ORDER BY t.id ASC
        `).all().map((row) => {
          let metadata = {};
          try {
          ...
          return {
            ...
            title: row.title,
            text: row.text,
            proposedValue: typeof metadata.proposedValue === 'string' ? metadata.proposedValue : '',
        ...
        res.end(JSON.stringify({
          ok: false,
          error: error instanceof Error ? error.message : 'Failed to read host development queue',
        }));
      }
      return;
    }
    ...
    if (req.method === 'POST' && parsedUrl.pathname === '/api/internal/config-apply-orchestrator/enqueue') {
      ...
      try {
        ...
        const db = getChatStatusDb();
        const enqueued = enqueueConfigApplyTask(db, task);
        const { taskId, status } = enqueued.task;
        const shouldDispatch = status === 'queued';

        if (shouldDispatch && backgroundJobsDisabled) {
          await postInternal('/api/internal/config-apply-tasks/execute', {
            taskId,
          });
        ...
        res.statusCode = shouldDispatch ? 202 : 200;
        res.end(JSON.stringify({
          ok: true,
          taskId,
          duplicate: enqueued.duplicate,
          taskStatus: status,
          suggestionStatus: status === 'applied' ? 'accepted' : 'dispatched',
        }));
      } catch (error) {
        res.statusCode = error instanceof ConfigApplyTaskError
          ? error.statusCode
          : 500;
        res.end(JSON.stringify({
          ok: false,
  ...
  const phoneHttpsServer = phoneRuntime
    ? createHttpsServer(readPhoneTlsOptions(), requestHandler)
    : null;
  server.requestTimeout = 0;
  if (phoneHttpsServer) {
    phoneHttpsServer.requestTimeout = 0;
  }
  ...
  const handleUpgrade = async (request, socket, head) => {
    ...
    if (
      phoneRuntime
      && !phoneLoopbackAuth.authorizeRequest(request, {
        allowDirect: false,
        allowWeb: request.socket.encrypted === true,
      })
    ) {
    ...
    if (!phoneRuntime && pathname !== '/ws/feed' && !trustNetwork && !(await isTrustedSocket(request))) {
      socket.write('HTTP/1.1 403 Forbidden\r\n\r\n');
      socket.destroy();
      return;
    }

    if (phoneRuntime || pathname !== '/ws/feed') {
  ...
  phoneHttpsServer?.on('upgrade', handleUpgrade);
  ...
  reconcilePhoneCurationLogOnStartup();
  reconcilePhoneCodeFixQueueOnStartup();
  const startupConfigApplyTaskIds = reconcileConfigApplyTasksOnStartup();
  ...
  const listen = (listener, listenerPort) => new Promise((resolve, reject) => {
    const onError = (error) => {
    ...
    listener.listen(listenerPort, listenHost);
  });

  const listeners = [listen(server, port)];
  if (phoneHttpsServer) {
    listeners.push(listen(phoneHttpsServer, phoneHttpsPort));
  }
  ...
  if (phoneHttpsServer) {
    console.log(`> Android shell ready on https://${hostname}:${phoneHttpsPort}`);
  }
  ...
  console.log(`> Runtime profile ${runtimeProfile}; listen=${listenHost}; background-jobs=${backgroundJobsDisabled ? 'off' : 'on'}; adaptive-heartbeat=${adaptiveHeartbeatMode}`);
  ...
  ensureReflectionStatusFile();
  ensureMainChatSessionSeed();

  if (backgroundJobsDisabled) {
    console.log('> Redis/worker background startup jobs disabled for this runtime profile');
    void regeneratePreferenceContextOnStartup();
  } else {
    console.log('> Background queue/reflection timers moved to worker.js');
    startArrangeBackstopTimer();
    void regeneratePreferenceContextOnStartup();
  }
  if (adaptiveHeartbeatMode !== 'off') {
    startAdaptiveHeartbeatTimer();
  }
  // Dev-agent timers (code-fix watchdog/janitor/redispatch) removed: Evogent no longer
  // runs its own dev agents; development happens through the user's coding tools.
}).catch((error) => {
  console.error('Failed to start Evogent server:', error);
  process.exit(1);
});
