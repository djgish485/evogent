...
function readReleaseIdentity(cwd, readFileSyncImpl = fs.readFileSync) {
  try {
    const identityPath = path.join(cwd, '.evogent-release.json');
    const parsed = JSON.parse(readFileSyncImpl(identityPath, 'utf8'));
...
function readDeploymentIdentity(options = {}) {
  ...
  const release = readReleaseIdentity(cwd, readFileSyncImpl);

  const commit = normalizeOptionalString(release?.sourceCommitShort)
    || readCommandOutput('git', ['rev-parse', '--short', 'HEAD'], { cwd, execFileSync: execFileSyncImpl });
  const commitFull = normalizeOptionalString(release?.sourceCommit)
    || readCommandOutput('git', ['rev-parse', 'HEAD'], { cwd, execFileSync: execFileSyncImpl });
  ...
  return {
    ...
    buildId: normalizeOptionalString(release?.buildId) || readBuildId(cwd, readFileSyncImpl),
    commit,
    commitFull,
    releaseId: normalizeOptionalString(release?.releaseId),
    releaseFormat: Number.isInteger(release?.releaseFormat) ? release.releaseFormat : null,
  };
}
