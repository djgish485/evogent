const { areBackgroundJobsDisabled } = require('../src/lib/runtime-profile');
...
let queueDependencies = null;

function assertBackgroundQueueEnabled() {
  if (!areBackgroundJobsDisabled()) return;
  const error = new Error('Background queue is disabled for this runtime profile');
  error.code = 'EVOGENT_BACKGROUND_QUEUE_DISABLED';
  throw error;
}

function getQueueDependencies() {
  assertBackgroundQueueEnabled();
  if (!queueDependencies) {
    const { Queue } = require('bullmq');
    const IORedis = require('ioredis');
    queueDependencies = { Queue, IORedis };
  }
...
function createQueueConnection({ forWorker = false } = {}) {
  assertBackgroundQueueEnabled();
  const { IORedis } = getQueueDependencies();
...
function getBackgroundQueue() {
  assertBackgroundQueueEnabled();
  const { Queue } = getQueueDependencies();
