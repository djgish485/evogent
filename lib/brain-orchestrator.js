...
function isTimeoutErrorMessage(message) {
  return typeof message === 'string' && /\btimed out\b/i.test(message);
}
...
function isChatBackedCurateTask(task) {
  const metadata = task?.metadata && typeof task.metadata === 'object' ? task.metadata : null;
  return task?.priority === 'user_chat'
    && typeof metadata?.curationCommand === 'string'
    && metadata.curationCommand.trim().toLowerCase() === '/curate';
}
...
function createBrainOrchestrator(deps) {
  ...
  class BrainOrchestrator {
    constructor(sessionName = 'evogent-ephemeral') {
      ...
      this.curationPromptPath = dataPath('curation-prompt.md');
      ...
      this.curationSession = this._normalizeBackgroundSessionState(persistedState.curationSession);
      this.curationSessionId = this.curationSession?.id || null;
      ...
      return new Promise((resolve) => {
        try {
          ...
          child.on('error', (err) => {
            ...
            console.error(`[orchestrator] WARNING: ${provider.displayName} not found in PATH. Chat and curation will not work. (${err.message})`);
          ...
          child.on('close', (code) => {
            if (code === 0) {
              ...
              this.brainAvailable = false;
              console.error(`[orchestrator] WARNING: ${provider.displayName} check failed (exit code ${code}). Chat and curation will not work.`);
            }
            this.emitStatus('brain_availability_changed');
    ...
    _resolveBackgroundSessionMaxAgeMs() {
      const rawDays = process.env.MEDIA_AGENT_CURATION_SESSION_MAX_AGE_DAYS
        || process.env.CURATION_SESSION_MAX_AGE_DAYS
        || '7';
      const parsedDays = Number.parseFloat(rawDays);
    ...
    _normalizeBackgroundSessionState(session) {
      ...
      let trackedFileMtimes = this._normalizeTrackedFileMtimes(session.trackedFileMtimes);
      if (Object.keys(trackedFileMtimes).length === 0) {
        const promptMtimeMs = typeof session.promptMtimeMs === 'number' && Number.isFinite(session.promptMtimeMs)
          ? session.promptMtimeMs
          : null;
        if (promptMtimeMs !== null) {
          trackedFileMtimes = {
            curationPrompt: promptMtimeMs,
          };
        }
      }
      ...
      try {
        if (fs.existsSync(this.historyFile)) {
          ...
          if (data && typeof data === 'object') {
            ...
            return {
              history,
              curationSession: this._normalizeBackgroundSessionState(data.curationSession),
    ...
    _saveHistory() {
      try {
        fs.writeFileSync(this.historyFile, JSON.stringify({
          ...
          curationSession: this.curationSession,
    ...
    _buildTaskLifecycleEvent(task) {
      ...
      return {
        ...
        chatMessageId: getTaskChatMessageId(task),
        sessionId: getTaskSessionId(task),
        curationLogRequestId: typeof metadata?.curationLogRequestId === 'string'
          ? metadata.curationLogRequestId.trim()
          : null,
        curationTriggeredBy: typeof metadata?.curationTriggeredBy === 'string'
          ? metadata.curationTriggeredBy.trim()
          : null,
        curationCommand: typeof metadata?.curationCommand === 'string'
          ? metadata.curationCommand.trim()
          : null,
    ...
    _resolveTaskPriorityValue(priority, metadata) {
      if (
        ...
        return PRIORITY_VALUES.heartbeat ?? PRIORITY_VALUES.user_chat ?? 0;
      }
    ...
    _getTerminalBrowseCacheRefreshRun(task) {
      ...
      try {
        ...
        return db.prepare(`
          ...
          LIMIT 1
        `).get(source, minRunTimestampMs, minRunTimestampMs) || null;
      } catch (error) {
        console.warn(`[orchestrator] failed to inspect browse-cache refresh state for ${task.id}: ${error.message}`);
        return null;
      }
    }
    ...
    async _terminateCacheRefreshTaskForRestart(task, {
      ...
      const terminalStatus = typeof terminalRun?.status === 'string'
        ...
        : null;
      const terminalSucceeded = terminalStatus === 'completed';
    ...
    _getCurationSessionTrackedFileMtimes() {
      return this._readTrackedFileMtimes({
        curationPrompt: this.curationPromptPath,
    ...
    _setBackgroundSessionState(kind, session) {
      ...
      if (kind === 'curation') {
        this.curationSession = normalizedSession;
        this.curationSessionId = normalizedSession?.id || null;
    ...
    _resolveBackgroundSession(task, providerName) {
      const sessionKind = isCurationTask(task)
        ? 'curation'
        : task?.priority === 'reflection'
          ? 'reflection'
          : null;
      if (!sessionKind) return null;

      const metadata = task?.metadata && typeof task.metadata === 'object' ? task.metadata : null;
      const explicitReset = sessionKind === 'curation'
        ? metadata?.forceFreshCurationSession === true || metadata?.resetCurationSession === true
        : metadata?.forceFreshReflectionSession === true || metadata?.resetReflectionSession === true;
      const trackedFileMtimes = sessionKind === 'curation'
        ? this._getCurationSessionTrackedFileMtimes()
        : this._getReflectionSessionTrackedFileMtimes();
      const existingSession = sessionKind === 'curation'
        ? this.curationSession
    ...
    _persistBackgroundSessionState(kind, providerName, activeSessionId) {
      ...
      const existingSession = kind === 'curation'
        ? this.curationSession
        : this.reflectionSession;
      const trackedFileMtimes = kind === 'curation'
        ? this._getCurationSessionTrackedFileMtimes()
        : this._getReflectionSessionTrackedFileMtimes();
    ...
    _launchBackgroundTask(task, {
      ...
      const deadlineAt = kind === 'curation'
        ? resolveTaskDeadlineAt(startedAt, timeoutMs)
        : null;
      const persistDeadlineAt = kind === 'curation'
      ...
      if (kind === 'curation') {
        writeCurationStatus({
          ...
          logFile,
          phase: 'caching',
          phaseDetail: 'Refreshing browse caches before curation',
        ...
        this.emitStatus('curation_phase_changed', {
          ...
          taskId: task.id,
          phase: 'caching',
          detail: 'Refreshing browse caches before curation',
        });

        void appendAgentEventToChatOutput({
          id: `event-curation-started-${task.id}`,
          text: 'Curation cycle started.',
          metadata: {
      ...
      const runTask = async () => {
        if (kind === 'curation') {
          await regeneratePreferenceContextBeforeCuration(task);
          console.log(`[orchestrator] starting background curation task ${task.id}`);
        } else if (kind === 'post_enrichment') {
        ...
        const onSpawn = (pid) => {
          if (kind === 'curation') {
            ...
            this.emitStatus('curation_phase_changed', {
              event: 'curation_phase',
              taskId: task.id,
              phase: readCurationStatus().phase || 'curating',
      ...
      return runTask()
        ...
        .finally(() => {
          ...
          if (kind === 'curation') {
            ...
            const failedBeforeSubmit = !curationStatus.submittedAt;
            const rawTaskError = task.error;
            let curationNotificationStatus = taskSucceeded ? 'completed' : 'failed';

            if (!taskSucceeded) {
              const curationFailureMessage = buildCurationFailureErrorMessage(curationStatus, task.error, { timeoutMs });

              if (!failedBeforeSubmit && isTimeoutErrorMessage(rawTaskError)) {
                task.state = 'completed';
                task.error = null;
                taskSucceeded = true;
                curationNotificationStatus = 'completed_with_warning';
                console.warn(`[orchestrator] curation task ${task.id} timed out after submit; treating as completed with warning`);
              } else {
                task.error = curationFailureMessage;
              }
            }

            if (curationStatus.phaseTaskId === task.id || !curationStatus.active || curationStatus.phase !== 'enriching') {
              writeCurationStatus({
                ...curationStatus,
                active: false,
                pid: null,
                ...
                phase: taskSucceeded ? 'completed' : curationStatus.phase,
                phaseDetail: taskSucceeded
                  ? (curationNotificationStatus === 'completed_with_warning'
                    ? 'Curation cycle completed after submit timeout'
                    : 'Curation cycle completed')
                  : curationStatus.phaseDetail,
                phaseUpdatedAt: taskSucceeded ? completedAt : curationStatus.phaseUpdatedAt,
                deadlineAt: curationStatus.deadlineAt || deadlineAt,
                persistDeadlineAt: curationStatus.persistDeadlineAt || persistDeadlineAt,
                lastFailureAt: taskSucceeded ? null : completedAt,
                lastFailurePhase: taskSucceeded ? null : curationStatus.phase,
                lastFailureDetail: taskSucceeded ? null : task.error,
                failedBeforeSubmit: taskSucceeded ? false : failedBeforeSubmit,
                cancelRequestedAt: null,
                cancelRequestedTaskId: null,
                cacheSkipRequestedAt: null,
              });
              this.emitStatus('curation_phase_changed', {
                event: 'curation_phase',
                taskId: task.id,
                phase: taskSucceeded ? 'completed' : curationStatus.phase || null,
                detail: taskSucceeded
                  ? (curationNotificationStatus === 'completed_with_warning'
                    ? 'Curation cycle completed after submit timeout'
                    : 'Curation cycle completed')
                  : curationStatus.phaseDetail || null,
              });
            }
          ...
          if (isChatBackedTask) {
            this._finalizeChatBackedTask(task);
          }
    ...
    async runBackgroundTask({ message, priority, source, metadata, requestId, timeoutMs }) {
      ...
      const task = {
        ...
        timeoutMs: Number.isInteger(timeoutMs) && timeoutMs > 0
          ? Math.min(timeoutMs, TASK_TIMEOUT_MS_BY_PRIORITY.heartbeat)
          : undefined,
    ...
    enqueue({ message, priority, source, metadata, requestId, timeoutMs }) {
      ...
      const isAutomatedCuration = metadata
        && typeof metadata === 'object'
        && metadata.automatedCuration === true;
      const isCuration = normalizedPriority === 'heartbeat'
        || (normalizedPriority === 'user_ping' && isCurationInstruction(normalizedMessage));

      if (isCuration || isAutomatedCuration) {
        const alreadyRunning = Boolean(this.readCurationStatusFile()) || (
          this.currentTask
          && (this.currentTask.priority === 'heartbeat'
            || (this.currentTask.priority === 'user_ping' && isCurationInstruction(this.currentTask.message))
            || this.currentTask.metadata?.automatedCuration === true)
        );
        const alreadyQueued = this.queue.some((t) =>
          t.priority === 'heartbeat'
          || (t.priority === 'user_ping' && isCurationInstruction(t.message))
          || t.metadata?.automatedCuration === true);
        if (alreadyRunning || alreadyQueued) {
          console.log('[orchestrator] Skipping duplicate curation — one is already running or queued');
          return { ok: false, error: 'Curation already in progress or queued' };
        }
      }
      ...
      // Background tasks (curation, reflection) launch immediately without
    ...
    readCurationStatusFile() {
      ...
      if (status.active) {
        ...
        if (isCurationStatusMissingPidStale(status)) {
          writeCurationStatus({
            ...status,
            active: false,
            pid: null,
        ...
        return status.phaseTaskId || status.requestId || 'curation-active';
      }

      if (this.currentTask?.state === 'processing' && isCurationTask(this.currentTask)) {
    ...
    getStatus() {
      const provider = this._getBrainProvider();
      const curationStatus = readCurationStatus();
      const hasStaleCurationStatus = curationStatus.active && (
        (curationStatus.pid && !isPidRunning(curationStatus.pid))
        || isCurationStatusMissingPidStale(curationStatus)
      );
      if (hasStaleCurationStatus) {
        writeCurationStatus({ ...curationStatus, active: false, pid: null });
      }
      const activeChatTasks = [...this.activeChatTasks.values()]
        .sort((left, right) => {
          const leftTime = Date.parse(left.startedAt || left.enqueuedAt || 0);
      ...
      const activeCurationAgent = hasStaleCurationStatus ? null : (this.readCurationStatusFile() || null);
      ...
      return {
        ...
        queueDepth: this.queue.length,
        isProcessing: hasStaleCurationStatus ? false : (this._hasActiveTasks() || Boolean(curationStatus.active)),
        activeCurationAgent,
    ...
    _cancelBackgroundCurationTask(requestedTaskId = null) {
      const status = readCurationStatus();
      if (!status.active) {
        return null;
      }

      const activeTaskId = status.phaseTaskId || status.requestId || 'curation-active';
      if (requestedTaskId && activeTaskId !== requestedTaskId) {
        return null;
      }
      const nowIso = new Date().toISOString();
      const isCachingPhase = status.phase === 'caching' || !status.phase;
      writeCurationStatus({
        ...status,
        phaseDetail: isCachingPhase
          ? 'Cancelling cache refresh; starting curation with available cache'
          : status.phaseDetail,
        cacheSkipRequestedAt: isCachingPhase ? nowIso : status.cacheSkipRequestedAt,
        cancelRequestedAt: nowIso,
        cancelRequestedTaskId: activeTaskId,
      });

      const pid = status.pid;
      const signalBackgroundCuration = (signal) => {
        if (!Number.isInteger(pid) || pid <= 0) {
          return false;
        }
      ...
      if (Number.isInteger(pid) && pid > 0 && isPidRunning(pid)) {
        signalBackgroundCuration('SIGTERM');
        writeCurationStatus({
          ...readCurationStatus(),
          active: false,
          pid: null,
        ...
        const forcedKillTimer = setTimeout(() => {
          if (isPidRunning(pid)) {
            signalBackgroundCuration('SIGKILL');
          }
      ...
      this.emitStatus('task_cancel_requested', {
        taskId: activeTaskId,
        chatMessageId: null,
        sessionId: this.curationSessionId,
        ts: nowIso,
      });
    ...
    cancelCurrentTask(taskId = null) {
      ...
      const backgroundCancellation = this._cancelBackgroundCurationTask(requestedTaskId);
      if (backgroundCancellation) {
        return backgroundCancellation;
      }
    ...
    async processTask(task) {
      ...
      if (isUserChatTask && !isTestMessage) {
        ...
        // Let existing curation routing take precedence
        if (slashCommand && !isCurationInstruction(chatBody) && isBackgroundRoutedCommand(slashCommand)) {
          const chatMessageId = getTaskChatMessageId(task);
          const sessionId = getTaskSessionId(task);
          ...
          return {
            ...
            reEnqueue: {
              ...
              timeoutMs: TASK_TIMEOUT_MS_BY_PRIORITY.heartbeat,
            },
          };
        }
      }
      ...
      try {
        if (isChatBackedCurateTask(task)) {
          await regeneratePreferenceContextBeforeCuration(task);
        }
    ...
    async _runBrainTask(task, prompt, timeoutMs, {
      ...
      let finalResultText = '';
      const isChatBackedTask = this._isChatBackedTask(task);
      const backgroundSessionKind = isCurationTask(task) && !isChatBackedTask
        ? 'curation'
        : task.priority === 'reflection'
          ? 'reflection'
          : null;
      let activeSessionId = isChatBackedTask
        ? this._getStoredChatSessionId(provider.name)
        : backgroundSessionKind === 'curation'
          ? (this.curationSession?.provider === provider.name ? this.curationSessionId : null)
      ...
      const curationPersistDeadlineAt = isCurationTask(task)
        ? resolveCurationPersistDeadlineAt(task.startedAt || task.enqueuedAt || null, timeoutMs)
        : null;
      const spawnOptions = {
        ...
        env: {
          ...
          ...(curationPersistDeadlineAt ? {
            MEDIA_AGENT_CURATION_PERSIST_DEADLINE_AT: curationPersistDeadlineAt,
            MEDIA_AGENT_CURATION_PERSIST_RESERVE_MS: String(resolveCurationPersistReserveMs(timeoutMs)),
            MEDIA_AGENT_CURATION_PROGRESS_URL: `${
              process.env.MEDIA_AGENT_INTERNAL_BASE_URL
              || process.env.ORCHESTRATOR_INTERNAL_URL
              || `http://127.0.0.1:${process.env.PORT || '3001'}`
            }/api/internal/curation/progress`,
          } : {}),
          ...(invocation.env || {}),
        },
        detached: true,
      ...
      return new Promise((resolve, reject) => {
        ...
        const finalize = (code, signal, error = null) => {
          ...
          if (error || transcriptLogError) {
            ...
            const timeoutMessage = isCurationTask(task)
              ? buildCurationFailureErrorMessage(readCurationStatus(), `Task timed out after ${Math.round(timeoutMs / 1000)}s`, { timeoutMs })
              : `Task timed out after ${Math.round(timeoutMs / 1000)}s`;
            completionError = buildTaskFailure(new Error(timeoutMessage));
        ...
        if (isCurationTask(task) || isPostEnrichmentTask(task)) {
        ...
        timeoutTimer = setTimeout(() => {
          ...
          const timeoutMessage = isCurationTask(task)
            ? buildCurationFailureErrorMessage(readCurationStatus(), `Task timed out after ${Math.round(timeoutMs / 1000)}s`, { timeoutMs })
            : `Task timed out after ${Math.round(timeoutMs / 1000)}s`;
          addTranscriptLine(`[orchestrator] ${timeoutMessage}`);
          child.kill('SIGTERM');
          killTimer = setTimeout(() => {
            child.kill('SIGKILL');
          }, 5_000);
    ...
    _runQueuedTask(task) {
      ...
      void (async () => {
        ...
        try {
          ...
          if (!backgrounded && isChatBackedTask) {
            this._finalizeChatBackedTask(task);
          }
