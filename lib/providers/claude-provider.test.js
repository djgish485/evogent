...
function buildPostEnrichmentInvocation(brainConfig = {}) {
  return createProvider(brainConfig).buildInvocation({
...
test('ephemeral Claude invocations disable local session persistence without a resumable id', () => {
  const invocation = createProvider().buildInvocation({
    prompt: 'Use private context once',
    systemPrompt: 'Transient private context',
    task: {
      priority: 'user_chat',
      metadata: { contextKind: 'screen' },
    },
    sessionMode: {
      mode: 'ephemeral',
    },
  ...
  assert.strictEqual(invocation.promptViaStdin, 'Use private context once');
});
...
test('post enrichment tasks use Sonnet 4.6', () => {
  const invocation = buildPostEnrichmentInvocation();

  assert.equal(getFlagValue(invocation.args, '--model'), 'claude-sonnet-4-6');
});

test('post enrichment effort comes from brain config with high fallback', () => {
  for (const { brainConfig, expected } of [
    { brainConfig: { postEnrichmentEffort: 'low' }, expected: 'low' },
    { brainConfig: { postEnrichmentEffort: 'high' }, expected: 'high' },
    { brainConfig: {}, expected: 'high' },
  ]) {
    const invocation = buildPostEnrichmentInvocation(brainConfig);
...
test('non-cache runtime tasks keep Opus 4.7 routing', () => {
  ...
  for (const priority of ['user_chat', 'user_ping']) {
    const invocation = provider.buildInvocation({
      prompt: 'Do the work',
      systemPrompt: 'System prompt',
      task: {
