...
function extractClaudeAssistantContext(rawEvent) {
  ...
  const inputTokens = normalizePositiveInteger(usage.input_tokens) || 0;
  const cacheReadInputTokens = normalizePositiveInteger(usage.cache_read_input_tokens) || 0;
  const cacheCreationInputTokens = normalizePositiveInteger(usage.cache_creation_input_tokens) || 0;
...
function extractClaudeResultContext(rawEvent) {
  ...
  for (const [modelId, usage] of Object.entries(modelUsage)) {
    ...
    const normalizedModelId = typeof modelId === 'string' && modelId.trim()
      ? modelId.trim()
      : null;
    if (contextWindow === null && normalizedModelId === null) {
      continue;
    }
...
function createClaudeProvider(deps, brainConfig) {
  ...
  return {
    ...
    buildInvocation({ prompt, systemPrompt, task, sessionMode }) {
      ...
      const useOpus = task.priority === 'heartbeat'
        || task.priority === 'user_ping'
      ...
      if (isCacheRefreshTask) {
        ...
        args.push('--effort', CLAUDE_CACHE_REFRESH_EFFORT);
        ...
        args.push('--model', CLAUDE_OPUS_MODEL);
