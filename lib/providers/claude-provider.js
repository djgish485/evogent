...
function createClaudeProvider(deps, brainConfig) {
  ...
  return {
    ...
    buildInvocation({ prompt, systemPrompt, task, sessionMode }) {
      ...
      // Claude Code >= ~2.1.2xx ignores the prompt passed as a positional arg in --print mode when
      // stdin is empty ("I don't see a task or question"); it reads the prompt from stdin. Pass the
      // prompt via stdin (see promptViaStdin below + the orchestrator's child.stdin write) instead
      // of as an arg. Older claude accepted the arg, so this stays back-compatible either way.
      const args = [
        '-p',
      ];
      ...
      const requestedModel = task?.metadata && typeof task.metadata === 'object'
        && typeof task.metadata.claudeModel === 'string' && task.metadata.claudeModel.trim()
        ? task.metadata.claudeModel.trim()
        : null;
      ...
      if (isCacheRefreshTask) {
        // A production-shaped Sonnet-low cache-refresh benchmark found:
        //   22 tweets, 100% avatar, 77% media, 0/22 quote (no quotes in feed sample),
        //   $0.98, 138s. Compared with the no-effort baseline ($1.29/437s/14 tweets):
        ...
        //   Tier maps: Low -> low (cost-sensitive), Medium/High -> high.
        // Cache browsing is hidden background work. Calibration kept
        // prompt/tools/session mode fixed and changed only model/effort.
        ...
        // Manual Sonnet-low post-enrichment benchmark (single reply thread):
        ...
        // Post enrichment is per-item hot-path work after curation cycles.
        ...
        // Per-session model override (e.g. the main session pins the fast tier for
        // interactive phone actions); reasoning effort still comes from the session.
