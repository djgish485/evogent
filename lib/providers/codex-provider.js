...
function createCodexProvider(deps = {}, brainConfig) {
  ...
  return {
    ...
    binaryName: 'codex',
    config: brainConfig,
    // Codex thread ids are SERVER-assigned rollout ids (UUIDv7 from thread.started),
    // not client-chosen. A fabricated id can never be resumed ("no rollout found"), so
    // the orchestrator must clear rather than mint one on reset and let thread.started
    // persist the real id. Claude accepts a client --session-id, so it omits this flag.
    usesServerAssignedSessionId: true,
    ...
    buildInvocation({ prompt, systemPrompt, sessionMode, task }) {
      ...
      const usePrivateEphemeralStdin = sessionMode?.mode === 'ephemeral'
        && task?.metadata?.contextKind === 'screen';
      const args = sessionMode?.mode === 'resume'
        ? ['exec', 'resume']
        : ['exec'];
      if (usePrivateEphemeralStdin) {
        // Private one-shot tasks must not write a resumable rollout under ~/.codex/sessions.
        // Unsupported CLI versions fail closed on this flag instead of silently persisting.
        args.push('--ephemeral');
      }
      ...
      const requestedModel = isRecord(task?.metadata) ? task.metadata.codexModel : null;
      // Production-shaped Codex calibration kept prompts/tools/permission mode fixed
      // and changed only model_reasoning_effort.
      ...
      // A scheduler-owned benchmark-qualified route may select a model for this
      // invocation without rewriting the deployment's global chat model.
      const sessionModel = normalizeCodexModel(
        typeof requestedModel === 'string' && requestedModel.trim()
          ? requestedModel
          : brainConfig.codexModel,
      );
      ...
      } else if (usePrivateEphemeralStdin) {
        // Keep private one-shot context out of argv/process listings as well as rollout files.
      ...
      return {
        command: 'codex',
        args,
        ...(usePrivateEphemeralStdin ? { promptViaStdin: fullPrompt } : {}),
        env: {},
      };
    },
