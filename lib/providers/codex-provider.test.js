...
test('ephemeral Codex invocations disable local rollout persistence without a resume id', () => {
  ...
  assert.match(invocation.promptViaStdin, /Transient private context/);
  assert.doesNotMatch(invocation.args.join(' '), /Transient private context/);
});
...
test('task metadata model routes one invocation without changing the provider default', () => {
  const provider = createProvider();

  const invocation = provider.buildInvocation({
    prompt: 'Curate the current candidate set',
    systemPrompt: 'System prompt',
    task: {
      priority: 'user_chat',
      metadata: {
  ...
  assert.equal(provider.config.codexModel, 'gpt-5.5');
});
...
test('non-downshifted Codex tasks keep the configured reasoning effort', () => {
  ...
  for (const priority of ['user_chat', 'user_ping', 'reflection']) {
    const invocation = provider.buildInvocation({
