...
const { parseConfigTimeZone, readTimeZoneConfig } = require('./time-zone.js');
...
const BRAIN_CONFIG_BY_USAGE_LEVEL = {
  low: {
...
## OpenClaw
<!-- Optional local OpenClaw chat mirror settings. Leave gatewayUrl and token blank for auto-discovery from ~/.openclaw/openclaw.json. -->
openclaw.gatewayUrl:
openclaw.token:
...
function deriveCuratorModelFromUsageLevel(value) {
  const usageLevel = normalizeUsageLevel(value) || DEFAULT_USAGE_LEVEL;
  return BRAIN_CONFIG_BY_USAGE_LEVEL[usageLevel].curatorModel;
}

function deriveCacheRefreshEffortFromUsageLevel(value) {
  const usageLevel = normalizeUsageLevel(value) || DEFAULT_USAGE_LEVEL;
  return BRAIN_CONFIG_BY_USAGE_LEVEL[usageLevel].cacheRefreshEffort;
}

function derivePostEnrichmentEffortFromUsageLevel(value) {
  const usageLevel = normalizeUsageLevel(value) || DEFAULT_USAGE_LEVEL;
  return BRAIN_CONFIG_BY_USAGE_LEVEL[usageLevel].postEnrichmentEffort;
}
...
function parseBrainConfig(content) {
  ...
  const timeZoneConfig = parseConfigTimeZone(content);
  ...
  return {
    ...
    curatorModel: deriveCuratorModelFromUsageLevel(usageLevel),
    cacheRefreshEffort: deriveCacheRefreshEffortFromUsageLevel(usageLevel),
    postEnrichmentEffort: derivePostEnrichmentEffortFromUsageLevel(usageLevel),
