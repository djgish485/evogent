...
async function markFailedBatchEnrichmentItems(task, { internalBaseUrl, fetchFn = globalThis.fetch, logger = console } = {}) {
  ...
  await Promise.all(postIds.map(async (postId) => {
    try {
      const itemUrl = `${internalBaseUrl}/api/feed/${encodeURIComponent(postId)}`;
      assertSafeInternalUrl(itemUrl);
      const getResponse = await fetchFn(itemUrl, {
        method: 'GET',
      ...
      const patchResponse = await fetchFn(itemUrl, {
        method: 'PATCH',
