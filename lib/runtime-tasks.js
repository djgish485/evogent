...
const { CACHE_SOURCE_SKILLS } = require('./cache-refresh-config');
...
function resolveCacheRefreshSkillSlug(source) {
  const normalizedSource = normalizeSourceArgument(source);
  const match = CACHE_SOURCE_SKILLS.find((entry) => entry.source === normalizedSource);
  return match?.skillSlug || (normalizedSource ? `${normalizedSource}-cache` : '');
}
...
function buildRuntimeTaskPrompt(task, options = {}) {
  ...
  if (isRuntimeCacheRefreshCommand(runtimeInstruction.commandName)) {
    ...
    if (cacheSource) {
      ...
      const skillSlug = resolveCacheRefreshSkillSlug(cacheSource);
      if (skillSlug) {
        resolvedValues.push(`MEDIA_AGENT_CACHE_REFRESH_SKILL_SLUG=${skillSlug}`);
      }
    }
    if (isSetupSourceSmokeTask(task)) {
      ...
      resolvedValues.push(`MEDIA_AGENT_CACHE_REFRESH_RUN_ID=${sanitizeRunIdPart(task.id)}`);
    }
  }
  ...
  return [
    ...
    isRuntimeCacheRefreshCommand(runtimeInstruction.commandName)
      ? 'For cache-refresh runs, use MEDIA_AGENT_CACHE_REFRESH_SOURCE as the requested source when present and MEDIA_AGENT_CACHE_REFRESH_SKILL_SLUG as the installed skill directory when present. If MEDIA_AGENT_CACHE_REFRESH_MODE=setup-smoke, run the bounded source-setup proof path and submit the provided run id and triggeredBy values.'
      : null,
    '',
