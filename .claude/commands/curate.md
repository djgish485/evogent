...
- Direct web article pre-submit gate:
  ...
  - For Hacker News articles read from `browse_cache_items.payload_json`, check `linkedArticleSynopsis` before a live fetch. If it has `status: "fetched"` and is not stale for the cache row, treat the article fetch gate as satisfied: use `ogDescription` first, then `leadParagraphs`, as the source-owned `text`/`excerpt` instead of re-fetching.
  - Only fetch a linked HN URL live when `linkedArticleSynopsis` is missing, stale, or not fetched. Do not use `Hacker News discussion: X points, Y comments.` as the `excerpt` for HN cards with external URLs; points and comments are metric/footer context, not article body copy.
