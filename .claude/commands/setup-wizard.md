...
1. **Optional agent name and manual Interests**: Read `data/config.md`. Treat missing Agent Name and the default `Evo` value as `optional`, not required; `data/config.md` still needs a sensible Agent Name by the end of setup. Treat manual Interests as optional backup/context when source, preference, archive, feedback, cache, or curation evidence exists; report interest inference as `ready` from that evidence. Treat manual Interests as a cold-start fallback only when no usable source/preference evidence exists.
...
- If the user accepts the offer to run the first curation or otherwise asks to run curation from setup chat, explain that OpenClaw owns scheduled curation and that accepted items are submitted through `evogent_feed_submit`. Do not enqueue an Evogent-native curation task.
