Run a background research task using Claude (Opus 4.6). Results are logged and you'll be notified when done.

Usage: /research-claude <topic or question>

Examples:
  /research-claude how does the Twitter API v2 rate limiting work
  /research-claude best practices for Next.js ISR caching
...
4. Run: `bash "$REPO_ROOT/scripts/agents/spawn-research.sh" "<prompt>" "<research-id>" "claude"`
5. Report the Research ID and log location
6. Report this status sentence: "Research is running in the background. Check /status for progress, or read the output at the log path when done."
...
CRITICAL — Script argument order is: "<prompt>" "<research-id>" "<agent-type>"
  ...
  - Arg 3: "claude" (REQUIRED — always pass this)
